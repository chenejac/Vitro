...
public class TBoxUpdater {
    ...
    private static final String classGroupURI =
        "http://vitro.mannlib.cornell.edu/ns/vitro/0.7#ClassGroup";
    private static final String inClassGroupURI =
    ...
    private Resource classGroupClass =
        (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM)).createResource(classGroupURI);
    private Property inClassGroupProp =
        (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM)).createProperty(inClassGroupURI);
    ...
    public void updateDefaultAnnotationValues(String subjectURI) throws IOException {
        ...
        try {
            ...
            Resource subj =
                (subjectURI == null) ? null : ResourceFactory.createResource(subjectURI);
            ...
            while (iter.hasNext()) {
                ...
                NodeIterator newObjects =
                    newTboxAnnotationsModel.listObjectsOfProperty(subject, predicate);
                ...
                if ((newObjects == null) || (!newObjects.hasNext())) {
                    ...
                    List<RDFNode> siteObjects =
                        siteModel.listObjectsOfProperty(subject, predicate).toList();
                    ...
                    if (siteObjects.size() > 0) {
                        ...
                        if (siteNode.equals(oldObject)) {
                            retractions
                                .add(siteModel.listStatements(subject, predicate, (RDFNode) null));
                        }
                    }
                    ...
                    try {
                        ...
                        if (detailLogs) {
                            logger.log("adding Statement: subject = " + subject.getURI() +
                                ...
                                " object = " +
                                (newObject.isLiteral() ? ((Literal) newObject).getLexicalForm()
                                    : ((Resource) newObject).getURI()));
                        }
                    } catch (Exception e) {
                        logger.logError(
                            "Error trying to add statement with property " + predicate.getURI() +
                ...
                if (!newObject.equals(oldObject)) {
                    ...
                    if (siteObject.equals(oldObject)) {
                        try {
                            StmtIterator it =
                                siteModel.listStatements(subject, predicate, (RDFNode) null);
                            while (it.hasNext()) {
                                retractions.add(it.next());
                            }
                        } catch (Exception e) {
                            logger.logError(
                                "Error removing statement for subject = " + subject.getURI() +
                                    "and property = " + predicate.getURI() +
                        ...
                        try {
                            ...
                            if (detailLogs) {
                                logger.log("Changed the value of property " + predicate.getURI() +
                                    ...
                                    " from " +
                                    (oldObject.isResource() ? ((Resource) oldObject).getURI() :
                                        ((Literal) oldObject).getLexicalForm()) +
                                    " to " +
                                    (newObject.isResource() ? ((Resource) newObject).getURI() :
                                        ((Literal) newObject).getLexicalForm()) +
                                    " in the knowledge base:\n");
                            }
                        } catch (Exception e) {
                            logger.logError("Error trying to change the value of property " +
                                predicate.getURI() +
                                " of class = " + subject.getURI() + " in the knowledge base:\n" +
                                e.getMessage());
                        }
            ...
            if (numRemoved > 0) {
                logger.log("Removed " + numRemoved +
                    " outdated vitro annotation property setting" + ((numRemoved > 1) ? "s" : "") +
                    " from the knowledge base");
            }
            ...
            Model newAnnotationSettings =
                newTboxAnnotationsModel.difference(oldTboxAnnotationsModel);
                ...
                if (!siteModel.contains(stmt)) {
                    ...
                    if (detailLogs) {
                        logger.log("adding Statement: subject = " + stmt.getSubject().getURI() +
                            ...
                            " object = " + (stmt.getObject().isLiteral() ?
            ...
            if (newAnnotationSettingsToAdd.size() > 0) {
                ...
                logger.log("Added " + newAnnotationSettingsToAdd.size() +
                    " new annotation property setting" + (plural ? "s" : "") +
                    " to the knowledge base. This includes only " +
    ...
    public void updateAnnotationModel() throws IOException {
        ...
        try {
            ...
            StmtIterator iter =
                oldTboxAnnotationsModel.listStatements((Resource) null, RDF.type, classGroupClass);

            while (iter.hasNext()) {
                Statement stmt = iter.next();

                if (!newTboxAnnotationsModel.contains(stmt) &&
                    !usesGroup(siteModel, stmt.getSubject())) {
                    long pre = retractions.size();
                    retractions.add(siteModel
                        .listStatements(stmt.getSubject(), (Property) null, (RDFNode) null));
                    long post = retractions.size();
                    if ((post - pre) > 0) {
                        logger.log("Removed the " + stmt.getSubject().getURI() +
                            " ClassGroup from the annotations model");
                    }
                }
            }
    ...
    public void removeObsoleteAnnotations() throws IOException {
        ...
        Resource subj1 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createResource("http://vivoweb.org/ontology/florida#StatewideGoalAndFocusArea");
        Resource obj1 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createResource("http://vivoweb.org/ontology#vitroClassGrouptopics");

        Property subj2 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM)).createProperty(
            "http://vivoweb.org/ontology/florida#divisionOfSponsoredResearchNumber");
        Resource obj2 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createResource("http://vivoweb.org/ontology#vitroPropertyGroupidentifiers");

        Property subj3 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createProperty("http://vivoweb.org/ontology/florida#statewideGoalAndFocusArea");
        Resource obj3 = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createResource("http://vivoweb.org/ontology#vitroPropertyGroupoutreach");

        Property inPropertyGroupProp = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
            .createProperty("http://vitro.mannlib.cornell.edu/ns/vitro/0.7#inPropertyGroup");
        ...
        try {
            ...
            if (siteModel.contains(subj1, inClassGroupProp, obj1)) {
                ...
                logger.log(
                    "Removed statement " + ABoxUpdater.stmtString(subj1, inClassGroupProp, obj1) +
                        " from the knowledge base (assumed to be obsolete)");
            }

            if (siteModel.contains(subj2, inPropertyGroupProp, obj2)) {
                retractions.add(subj2, inPropertyGroupProp, obj2);
                logger.log("Removed statement " +
                    ABoxUpdater.stmtString(subj2, inPropertyGroupProp, obj2) +
                    " from the knowledge base (assumed to be obsolete)");
            }
            ...
            if (siteModel.contains(subj3, inPropertyGroupProp, obj3)) {
                ...
                logger.log("Removed statement " +
                    ABoxUpdater.stmtString(subj3, inPropertyGroupProp, obj3) +
                    " from the knowledge base (assumed to be obsolete)");
            }
    ...
    private void mergePropertyAnnotationsToPropertyConfig(AtomicOntologyChange changeObj,
                                                          Model userAnnotationsModel)
        throws IOException {
        String contextURI =
            VitroVocabulary.PROPERTY_CONFIG_DATA + changeObj.getNotes().substring(3);
