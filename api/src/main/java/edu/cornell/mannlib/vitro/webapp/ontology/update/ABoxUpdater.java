...
public class ABoxUpdater {
    ...
    private OntClass OWL_THING =
        (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM)).createClass(OWL.Thing.getURI());
    ...
    public static String stmtString(Resource subject, Property predicate, RDFNode object) {
        return " [subject = " + subject.getURI() +
            ...
            "] [object = " +
            (object.isLiteral() ? ((Literal) object).getLexicalForm() + " (Literal)"
    ...
    public void processClassChanges(List<AtomicOntologyChange> changes) throws IOException {
        ...
        for (AtomicOntologyChange change : changes) {
            switch (change.getAtomicChangeType()) {
                ...
                default:
                    logger.logError(
                        "unexpected change type indicator: " + change.getAtomicChangeType());
            }
        }
    }
    ...
    public void renameClass(AtomicOntologyChange change) throws IOException {
        ...
        while (graphIt.hasNext()) {
            ...
            try {
                ...
                StmtIterator iter =
                    aboxModel.listStatements(oldClass, (Property) null, (RDFNode) null);
                ...
                if (removeCount > 0) {
                    logger.log("Removed " + removeCount + " subject reference" +
                        ((removeCount > 1) ? "s" : "") + " to the " + oldClass.getURI() + " class");
                }
                ...
                while (iter.hasNext()) {
                    renameCount++;
                    Statement oldStatement = iter.next();
                    Statement newStatement = ResourceFactory
                        .createStatement(oldStatement.getSubject(), RDF.type, newClass);
                    retractions.add(oldStatement);
                ...
                if (renameCount > 0) {
                    logger.log(
                        "Retyped " + renameCount + " individual" + ((renameCount > 1) ? "s" : "") +
                            " from type " + oldClass.getURI() + " to type " + newClass.getURI());
                }
    ...
    public void addClass(AtomicOntologyChange change) throws IOException {
        ...
        if (addedClass == null) {
            logger.logError(
                "didn't find the added class " + change.getDestinationURI() + " in the new model.");
            return;
        }
            ...
            if (!parentOfAddedClass.equals(OWL.Thing)) {
                ...
                while (graphIt.hasNext()) {
                    ...
                    StmtIterator stmtIter =
                        aboxModel.listStatements(null, RDF.type, parentOfAddedClass);
                    ...
                    if (count > 0) {
                        ...
                        logger.log(
                            "There " + ((count > 1) ? "are" : "is") + " " + count + " individual" +
                                ((count > 1) ? "s" : "") + " in the model that " +
                                ((count > 1) ? "are" : "is") + " of type " +
                                parentOfAddedClass.getURI() + "," +
                                " and a new subclass of that class has been added: " +
                                addedClass.getURI() + ". " +
                                "Please review " + ((count > 1) ? "these" : "this") +
                                " individual" + ((count > 1) ? "s" : "") + " to see whether " +
                                ((count > 1) ? "they" : "it") + " should be of type: " +
                                addedClass.getURI());
                    }
    ...
    public void renameClassToParent(AtomicOntologyChange change) throws IOException {
        ...
        if (deletedClass == null) {
            logger.log("WARNING: didn't find the deleted class " + change.getSourceURI() +
                " in the old model. Skipping updates for this deletion");
            return;
        }
        ...
        //log summary of changes
        logger.log("Class " + deletedClass.getURI() +
            " has been deleted. Any references to it in the knowledge base have been changed to " +
            replacementClass.getURI());

        AtomicOntologyChange chg =
            new AtomicOntologyChange(deletedClass.getURI(), replacementClass.getURI(),
                AtomicChangeType.RENAME, change.getNotes());
        renameClass(chg);
    }
    ...
    public void deleteIndividualsOfType(AtomicOntologyChange change) throws IOException {
        ...
        while (graphIt.hasNext()) {
            ...
            try {
                ...
                int refCount = 0;
                StmtIterator iter =
                    aboxModel.listStatements((Resource) null, RDF.type, deletedClass);
                ...
                if (count > 0) {
                    logger.log("Removed " + count + " individual" +
                        (((count > 1) ? "s" : "") + " of type " + deletedClass.getURI()) +
                        " (refs = " + refCount + ")");
                }
    ...
    protected int deleteIndividual(Resource individual) throws IOException {
        ...
        while (graphIt.hasNext()) {
            ...
            try {
                StmtIterator iter =
                    aboxModel.listStatements(individual, (Property) null, (RDFNode) null);
        ...
        for (AtomicOntologyChange propChangeObj : changes) {
            ...
            try {
                ...
                switch (propChangeObj.getAtomicChangeType()) {
                    ...
                    default:
                        log.debug("unknown");
                        logger.logError("unexpected change type indicator: " +
                            propChangeObj.getAtomicChangeType());
                        break;
                }
            } catch (Exception e) {
                log.error(e, e);
    ...
    private void addProperty(AtomicOntologyChange propObj) throws IOException {
        ...
        if (addedProperty == null) {
            logger.logError(
                "Unable to find property " + propObj.getDestinationURI() + " in new TBox");
            return;
        }
        ...
        if (inverseOfAddedProperty != null) {
            ...
            while (graphIt.hasNext()) {
                ...
                try {
                    StmtIterator iter = aboxModel
                        .listStatements((Resource) null, inverseOfAddedProperty, (RDFNode) null);
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (stmt.getObject().isResource()) {
                            Statement newStmt = ResourceFactory
                                .createStatement(stmt.getObject().asResource(), addedProperty,
                                    stmt.getSubject());
                            additions.add(newStmt);
                        } else {
                            logger.log(
                                "WARNING: expected the object of this statement to be a Resource but it is not. No inverse has been asserted: " +
                                    stmtString(stmt));
                        }
                    }
    ...
    private void deleteProperty(AtomicOntologyChange propObj) throws IOException {
        ...
        if (deletedProperty == null && "Prop".equals(propObj.getNotes())) {
            deletedProperty = (ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM))
                .createOntProperty(propObj.getSourceURI());
        }

        if (deletedProperty == null) {
            logger.log("WARNING: didn't find deleted property " + propObj.getSourceURI() +
                " in oldTBoxModel");
            return;
        }
        ...
        while (graphIt.hasNext()) {
            ...
            if (replacementProperty == null) {
                ...
                try {
                    deletePropModel.add(
                        aboxModel.listStatements((Resource) null, deletedProperty, (RDFNode) null));
                ...
                if (deletePropModel.size() > 0) {
                    logger.log(
                        "Removed " + deletePropModel.size() + " statement" + (plural ? "s" : "") +
                            " with predicate " +
                            propObj.getSourceURI());
                }
            } else {
                AtomicOntologyChange chg =
                    new AtomicOntologyChange(deletedProperty.getURI(), replacementProperty.getURI(),
                        AtomicChangeType.RENAME, propObj.getNotes());
                renameProperty(chg);
            }
        }
    ...
    private void renameProperty(AtomicOntologyChange propObj) throws IOException {
        ...
        logger.log("Processing a property rename from: " + propObj.getSourceURI() + " to " +
        ...
        if (oldProperty == null) {
            logger.logError(
                "didn't find the " + propObj.getSourceURI() + " property in the old TBox");
            return;
        }

        if (newProperty == null) {
            logger.logError(
                "didn't find the " + propObj.getDestinationURI() + " property in the new TBox");
            return;
        }
...
//    		    log.info(System.currentTimeMillis() - start + " to run sparql construct for " + renamePropRetractModel.size() + " statements" );
                ...
                log.debug(System.currentTimeMillis() - start + " to list " +
                    renamePropRetractModel.size() + " old statements");
                start = System.currentTimeMillis();
