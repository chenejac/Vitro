    ...
    private void redirectToDownload(VitroRequest vreq, HttpServletResponse response) {
        ...
        try {
            response.sendRedirect(
                "./" + base + "/download/" + filename + extension + "?" + vreq.getQueryString());
        ...
        if ("abox".equals(subgraphParam)) {
            ...
            if ("inferred".equals(assertedOrInferredParam)) {
                ...
                Model inferenceModel =
                    ModelAccess.on(getServletContext()).getOntModel(TBOX_INFERENCES);
                ...
                model = xutil.extractTBox(
                    ModelAccess.on(getServletContext()).getOntModel(TBOX_ASSERTIONS),
                    ontologyURI.toString());
            }
            ...
            if ("inferred".equals(assertedOrInferredParam)) {
                ...
                ontModel
                    .addSubModel(ModelAccess.on(getServletContext()).getOntModel(ABOX_INFERENCES));
                ontModel
                    .addSubModel(ModelAccess.on(getServletContext()).getOntModel(TBOX_INFERENCES));
            } else if ("full".equals(assertedOrInferredParam)) {
        ...
        try {
            ...
            try {
                if (!"full".equals(subgraphParam)) {
                    ...
                    ontModel
                        .writeAll(outStream, "N3".equals(formatParam) ? "TTL" : formatParam, null);
                }
                ...
                log.info(
                    "exported model in N-TRIPLE format instead of N3 because there was a problem exporting in N3 format");
            }
