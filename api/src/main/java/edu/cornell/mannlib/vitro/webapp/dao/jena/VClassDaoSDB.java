    ...
    public void addVClassesToGroup(VClassGroup group, boolean includeUninstantiatedClasses,
        ...
        try {
            if ((group != null) && (group.getURI() != null)) {
                ...
                Property inClassGroup =
                    ResourceFactory.createProperty(VitroVocabulary.IN_CLASSGROUP);
                if (inClassGroup != null) {
                    StmtIterator annotIt =
                        getOntModel().listStatements((Resource) null, inClassGroup, groupRes);
                    try {
                                    ...
                                    if (getIndividualCount) {
                                        ...
                                        String countQueryStr =
                                            "SELECT COUNT(DISTINCT ?s) WHERE \n" +
                                                ...
                                                WebappDaoFactorySDB
                                                    .getFilterBlock(graphVars, datasetMode) +
                                                "} \n";
                                        Query countQuery =
                                            QueryFactory.create(countQueryStr, Syntax.syntaxARQ);
                                        DatasetWrapper w = getDatasetWrapper();
                                        Dataset dataset = w.getDataset();
                                        dataset.getLock().enterCriticalSection(Lock.READ);
                                        try {
                                            QueryExecution qe =
                                                QueryExecutionFactory.create(countQuery, dataset);
