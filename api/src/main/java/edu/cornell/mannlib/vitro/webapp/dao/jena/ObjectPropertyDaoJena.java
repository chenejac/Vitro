...
//        "   ?property a owl:ObjectProperty . \n" +
    ...
    static {
        try {
            ...
            log.error("could not create SPARQL query for LIST_VIEW_CONFIG_FILE_QUERY_STRING " +
                th.getMessage());
    ...
    public void fillObjectPropertiesForIndividual(Individual entity) {
        ...
        props.addAll(
            getObjectPropertiesForObjectPropertyStatements(entity.getObjectPropertyStatements()));
    ...
    protected ObjectProperty propertyFromOntProperty(OntProperty op) {
        ...
        try {
            ...
            if (op.getDomain() != null) {
                p.setDomainVClassURI((op.getDomain().isAnon()) ?
                    PSEUDO_BNODE_NS + op.getDomain().getId().toString() : op.getDomain().getURI());
            }
            if (op.getRange() != null) {
                p.setRangeVClassURI(
                    (op.getRange().isAnon()) ? PSEUDO_BNODE_NS + op.getRange().getId().toString() :
                        op.getRange().getURI());
            }
            ...
            RDFNode objectIndividualSortPropertyNode =
                op.getPropertyValue(PROPERTY_OBJECTINDIVIDUALSORTPROPERTY);
            if (objectIndividualSortPropertyNode instanceof Resource) {
                p.setObjectIndividualSortPropertyURI(
                    ((Resource) objectIndividualSortPropertyNode).getURI());
            }
            p.setDomainEntitySortDirection(
                getPropertyStringValue(op, PROPERTY_ENTITYSORTDIRECTION));
            p.setRangeEntitySortDirection(
                getPropertyStringValue(invOp, PROPERTY_ENTITYSORTDIRECTION));
            ...
            p.setProhibitedFromUpdateBelowRoleLevel(
            ...
            Boolean offerCreateObj =
                getPropertyBooleanValue(op, PROPERTY_OFFERCREATENEWOPTIONANNOT);
            p.setOfferCreateNewOption(offerCreateObj == null ? false : offerCreateObj);
            Boolean dependencyDeletionObj =
                getPropertyBooleanValue(op, PROPERTY_STUBOBJECTPROPERTYANNOT);
            p.setStubObjectRelation(dependencyDeletionObj == null ? false : dependencyDeletionObj);

            Boolean collateBySubclass =
                getPropertyBooleanValue(op, PROPERTY_COLLATEBYSUBCLASSANNOT);
            ...
            Boolean deleteLinkSuppressed =
                getPropertyBooleanValue(op, PROPERTY_DELETELINKSUPPRESSED);
    ...
    public List getAllObjectProperties() {
        ...
        try {
            ...
            try {
                ...
                try {
                    while (opIt.hasNext()) {
                        org.apache.jena.ontology.ObjectProperty op =
                            (org.apache.jena.ontology.ObjectProperty) opIt.next();
                ...
                ClosableIterator opIt =
                    getOntModel().listSubjectsWithProperty(RDF.type, RDF.Property);
                try {
                    while (opIt.hasNext()) {
                        Resource res = (Resource) opIt.next();
                        if ((res.canAs(OntProperty.class)) &&
                            (!NONUSER_NAMESPACES.contains(res.getNameSpace()))) {
    ...
    public ObjectProperty getObjectPropertyByURIs(String propertyURI,
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                "Getting " + propertyURI + " with domain " + domainURI + " and range " + rangeURI);
        }
        ...
        propQuery += "    OPTIONAL { ?context config:qualifiedByRoot ?rangeRoot } \n" +
            ...
            "    OPTIONAL { ?configuration <" + PROPERTY_COLLATEBYSUBCLASSANNOT.getURI() +
            "> ?collateBySubclass } \n" +
            "    OPTIONAL { ?configuration <" + DISPLAY_LIMIT.getURI() + "> ?displayLimit } \n" +
            "    OPTIONAL { ?configuration <" + PROPERTY_OBJECTINDIVIDUALSORTPROPERTY.getURI() +
            "> ?individualSortProperty } \n " +
            "    OPTIONAL { ?configuration <" + PROPERTY_ENTITYSORTDIRECTION.getURI() +
            "> ?entitySortDirection } \n" +
            "    OPTIONAL { ?configuration <" + PROPERTY_SELECTFROMEXISTINGANNOT.getURI() +
            "> ?selectFromExisting } \n" +
            "    OPTIONAL { ?configuration <" + PROPERTY_OFFERCREATENEWOPTIONANNOT.getURI() +
            "> ?offerCreateNew } \n" +
            "    OPTIONAL { ?configuration <" + PUBLIC_DESCRIPTION_ANNOT.getURI() +
            "> ?publicDescription } \n" +
            "    OPTIONAL { ?configuration <" + PROPERTY_STUBOBJECTPROPERTYANNOT.getURI() +
            "> ?stubDeletion } \n" +
            "}";

        Query q = QueryFactory.create(propQuery);
        QueryExecution qe =
            QueryExecutionFactory.create(q, getOntModelSelector().getDisplayModel());
        try {
    ...
    public List<ObjectProperty> getObjectPropertiesForObjectPropertyStatements(
        List<ObjectPropertyStatement> objPropertyStmts) {
    ...
    public int insertProperty(ObjectProperty prop, OntModel ontModel) throws InsertException {
        ...
        try {
            getOntModel().getBaseModel()
                .notifyEvent(new EditEvent(getWebappDaoFactory().getUserURI(), true));
            String errMsgStr = getWebappDaoFactory().checkURIForEditableEntity(prop.getURI());
            if (errMsgStr != null) {
                throw new InsertException(errMsgStr);
            }
            org.apache.jena.ontology.ObjectProperty p =
                ontModel.createObjectProperty(prop.getURI());
            ...
            getOntModel().getBaseModel()
                .notifyEvent(new EditEvent(getWebappDaoFactory().getUserURI(), false));
            ontModel.leaveCriticalSection();
        }
    }
    ...
    private void doUpdate(ObjectProperty prop, OntProperty p, OntProperty inv, OntModel ontModel) {
        ...
        try {
            ...
            if (parentURI != null) {
                org.apache.jena.ontology.ObjectProperty parent =
                    ontModel.getObjectProperty(prop.getParentURI());
                if (parent != null) {
        ...
        updatePropertyNonNegativeIntegerValue(p, DISPLAY_LIMIT, prop.getDomainDisplayLimitInteger(),
            getOntModel());
        updatePropertyStringValue(p, PROPERTY_ENTITYSORTDIRECTION,
            prop.getDomainEntitySortDirection(), getOntModel());
        if (inv != null) {
            ...
            updatePropertyNonNegativeIntegerValue(inv, DISPLAY_LIMIT,
                prop.getRangeDisplayLimitInteger(), getOntModel());
            updatePropertyStringValue(inv, PROPERTY_ENTITYSORTDIRECTION,
                prop.getRangeEntitySortDirection(), getOntModel());
        }

        updatePropertyNonNegativeIntegerValue(p, DISPLAY_RANK_ANNOT,
            prop.getDomainDisplayTierInteger(), getOntModel());
        if (inv != null) {
            updatePropertyNonNegativeIntegerValue(inv, DISPLAY_RANK_ANNOT,
                prop.getRangeDisplayTierInteger(), getOntModel());
        }
        ...
        if ((oldObjectIndividualSortPropertyURI != null &&
            prop.getObjectIndividualSortPropertyURI() == null) ||
            (oldObjectIndividualSortPropertyURI != null &&
                prop.getObjectIndividualSortPropertyURI() != null &&
                !(oldObjectIndividualSortPropertyURI
                    .equals(prop.getObjectIndividualSortPropertyURI())))) {
        ...
        if ((oldObjectIndividualSortPropertyURI == null &&
            ...
            try {
                if (prop.getObjectIndividualSortPropertyURI().length() > 0) {
                    newObjectIndividualSortProperty =
                        ResourceFactory.createResource(prop.getObjectIndividualSortPropertyURI());
                }
            } catch (Exception e) {
            }
            if (newObjectIndividualSortProperty != null) {
                p.addProperty(PROPERTY_OBJECTINDIVIDUALSORTPROPERTY,
                    newObjectIndividualSortProperty);
            }
        }

        if (prop.getHiddenFromDisplayBelowRoleLevel() != null) {
            updatePropertyResourceURIValue(p, HIDDEN_FROM_DISPLAY_BELOW_ROLE_LEVEL_ANNOT,
                prop.getHiddenFromDisplayBelowRoleLevel().getURI());
        }

        if (prop.getProhibitedFromUpdateBelowRoleLevel() != null) {
            updatePropertyResourceURIValue(p, PROHIBITED_FROM_UPDATE_BELOW_ROLE_LEVEL_ANNOT,
                prop.getProhibitedFromUpdateBelowRoleLevel().getURI());
        }

        if (prop.getHiddenFromPublishBelowRoleLevel() != null) {
            updatePropertyResourceURIValue(p, HIDDEN_FROM_PUBLISH_BELOW_ROLE_LEVEL_ANNOT,
                prop.getHiddenFromPublishBelowRoleLevel().getURI());
        }

        updatePropertyStringValue(p, PROPERTY_CUSTOMENTRYFORMANNOT, prop.getCustomEntryForm(),
            ontModel);
        updatePropertyBooleanValue(p, PROPERTY_SELECTFROMEXISTINGANNOT,
            prop.getSelectFromExisting(), ontModel, JenaBaseDao.KEEP_ONLY_IF_FALSE);
        updatePropertyBooleanValue(p, PROPERTY_OFFERCREATENEWOPTIONANNOT,
            prop.getOfferCreateNewOption(), ontModel, JenaBaseDao.KEEP_ONLY_IF_TRUE);
        updatePropertyBooleanValue(p, PROPERTY_STUBOBJECTPROPERTYANNOT,
            prop.getStubObjectRelation(), ontModel, JenaBaseDao.KEEP_ONLY_IF_TRUE);
        updatePropertyBooleanValue(p, PROPERTY_COLLATEBYSUBCLASSANNOT, prop.getCollateBySubclass(),
    ...
    public void deleteProperty(ObjectProperty prop, OntModel ontModel) {
        ...
        try {
            ...
            if ((prop.getURIInverse() != null) && (prop.getURIInverse().length() > 0)) {
                ...
                if (invOp != null) {
                    Iterator<Resource> restIt =
                        ontModel.listSubjectsWithProperty(OWL.onProperty, op);
                    while (restIt.hasNext()) {
                        Resource restRes = restIt.next();
                        if (restRes.canAs(OntResource.class)) {
                            OntResource restOntRes = restRes.as(OntResource.class);
                        ...
                        if (parentIt != null) {
                                    ...
                                    if (parentStmt.getObject().isResource()) {
                                        ...
                                        if (parentRes.getURI() != null) {
                                            Property parentProperty =
                                                ResourceFactory.createProperty(parentRes.getURI());
                            ...
                            if (parentList.size() == 0) {
                                ...
                                for (Property pt : parentList) {
                                    if ((!pt.equals(op)) && (!(getOntModel()
                                        .contains(op, OWL.equivalentProperty, pt)) ||
                                        (getOntModel().contains(pt, OWL.equivalentProperty, op)))) {
                                        isRoot = false;
                                    }
                                }
                            }
    ...
    public List<String> getSuperPropertyURIs(String objectPropertyURI) {
        ...
        try {
            Iterator supIt =
                getOntModel().getOntProperty(objectPropertyURI).listSuperProperties(true);
            while (supIt.hasNext()) {
                try {
    ...
    public List<String> getSubPropertyURIs(String objectPropertyURI) {
        ...
        try {
            Iterator subIt =
                getOntModel().getOntProperty(objectPropertyURI).listSubProperties(true);
            while (subIt.hasNext()) {
                try {
    ...
    public List<ObjectProperty> getObjectPropertyList(String subjectUri) {
        ...
        // subject uri instead.
        String queryString =
            QueryUtils.subUriForQueryVar(OBJECT_PROPERTY_QUERY_STRING, "subject", subjectUri);
        ...
        getPropertyQueryResults(queryString, new ResultSetConsumer() {
            ...
            protected void processQuerySolution(QuerySolution qs) {
                ...
                log.debug(
                    "Found populated object property " + uri + " with object type " + objType);
    ...
    public String getCustomListViewConfigFileName(ObjectProperty op) {
        ...
        if (customListViewConfigFileMap.isEmpty()) {
            ...
            QueryExecution qexec =
                QueryExecutionFactory.create(listViewConfigFileQuery, displayModel);
