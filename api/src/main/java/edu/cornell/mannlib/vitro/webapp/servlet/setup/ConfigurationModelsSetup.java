...
public class ConfigurationModelsSetup implements ServletContextListener {
    ...
    private void setupModel(ServletContext ctx, String modelUri, String modelPath) {
        try {
            ...
            if (ontModel.isEmpty()) {
                ...
                OntModel baseModelFirsttime =
                    ModelAccess.on(ctx).getOntModel(modelUri + "FirsttimeBackup");
            ...
            throw new RuntimeException(
                "Failed to create the '" + modelPath + "' model (" + modelUri + ").", e);
        }
    }
     ...
     */
    private void applyFirstTimeChanges(ServletContext ctx, String modelPath, String modelUri,
                                       OntModel userModel) {
        ...
        OntModel baseModelFirsttimeBackup =
            ModelAccess.on(ctx).getOntModel(modelUri + "FirsttimeBackup");
        ...
        if (RDFFilesLoader.areIsomporphic(baseModelFirsttime, baseModelFirsttimeBackup)) {
            ...
            updatedFiles =
                applyChanges(baseModelFirsttimeBackup, baseModelFirsttime, userModel, modelPath);
            if (updatedFiles) {
