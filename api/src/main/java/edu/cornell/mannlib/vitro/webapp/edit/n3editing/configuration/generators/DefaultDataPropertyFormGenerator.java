...
public class DefaultDataPropertyFormGenerator extends BaseEditConfigurationGenerator
    ...
    public static void prepareForDataPropUpdate(Model model,
        ...
        if (dataHash == null) {
            throw new Error(
                "prepareForDataPropUpdate() should not be called if the EditConfiguration is not a data property statement update ");
        } else {
            dps = RdfLiteralHash.getPropertyStmtByHash(subjectUri, predicateUri, dataHash, model);
            if (dps == null) {
                throw new Error("No match to existing data property \"" + predicateUri +
                    "\" statement for subject \"" + subjectUri + "\" via key " + dataHash);
            } else {
    ...
    private static void dataTypeDebug(DataPropertyStatement dps,
        ...
        if (statementLang == null) {
            log.debug(
                "no language attribute on data property statement in DefaultDataPropertyFormGenerator");
        } else {
            log.debug("language attribute of [" + statementLang +
                "] on data property statement in DefaultDataPropertyFormGenerator");
        }
        ...
        if (statementDataType == null) {
            log.debug(
                "no range datatype uri set on data property statement when property's range datatype is " +
                    dataproperty.getRangeDatatypeURI() + " in DefaultDataPropertyFormGenerator");
        } else {
            log.debug("range datatype uri of [" + statementDataType +
                "] on data property statement in DefaultDataPropertyFormGenerator");
        }
    }
    ...
    public EditConfigurationVTwo getEditConfiguration(VitroRequest vreq, HttpSession session) {
        ...
        if (subject == null) {
            throw new Error(
                "In DefaultDataPropertyFormGenerator, could not find individual for URI " +
                    subjectUri);
        }
        ...
        DataProperty dataproperty =
            unfilteredWdf.getDataPropertyDao().getDataPropertyByURI(predicateUri);

        dataproperty =
            vreq.getWebappDaoFactory().getDataPropertyDao().getDataPropertyByURI(predicateUri);
        if (dataproperty == null) {
        ...
        if (rangeDatatypeUri == null || rangeDatatypeUri.trim().isEmpty()) {
            rangeDatatypeUri = vreq.getWebappDaoFactory().getDataPropertyDao()
                .getRequiredDatatypeURI(subject, dataproperty);
        }
        ...
        if (rangeDatatypeUri != null) {
            editConfiguration
                .addValidator(new DefaultDataPropertyFormValidator(rangeDatatypeUri, vreq));
        }
        //prepare
