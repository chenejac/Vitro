...
public class EntityEditController extends BaseEditController {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        Individual inferredEnt =
            vreq.getUnfilteredWebappDaoFactory().getIndividualDao().getIndividualByURI(entURI);
        if (inferredEnt == null) {
        ...
        String rModTime =
            (ent.getModTime() == null) ? "" : publicDateFormat.format(ent.getModTime());
        ...
        request.setAttribute("types",
            types); // we're displaying all assertions, including indirect types
        ...
        try {
            ...
            if (ent.getExternalIds() != null) {
                for (DataPropertyStatement eid : ent.getExternalIds()) {
                    String multiplexedString = "DatapropURI:" +
                        new String(Base64.encodeBase64(eid.getDatapropURI().getBytes())) + ";" +
        ...
        try {
            ...
            for (PropertyInstance pi : epiColl) {
                String multiplexedString = "PropertyURI:" +
                    new String(Base64.encodeBase64(pi.getPropertyURI().getBytes())) + ";" +
                    "ObjectEntURI:" +
                    new String(Base64.encodeBase64(pi.getObjectEntURI().getBytes()));
                epiOptionList.add(
                    new Option(multiplexedString, pi.getDomainPublic() + " " + pi.getObjectName()));
            }
        ...
        try {
            ...
            OptionMap.put("PropertyURI", FormUtils
                .makeOptionListFromBeans(piList, "PropertyURI", "DomainPublic", (String) null,
                    (String) null, false));
        } catch (Exception e) {
            log.error(e, e);
        }
        ...
        request.setAttribute("css",
            "<link rel=\"stylesheet\" type=\"text/css\" href=\"" + application.getThemeDir() +
                "css/edit.css\"/>");
        request.setAttribute("scripts", "/templates/edit/specific/ents_edit_head.jsp");

        try {
            JSPPageHandler
                .renderBasicPage(request, response, "/templates/edit/specific/ents_edit.jsp");
        } catch (Exception e) {
            log.error("EntityEditController could not forward to view.");
            log.error(e.getMessage());
            log.error(e.getStackTrace());
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        log.trace("Please don't POST to the " + this.getClass().getName() +
            ". Use GET instead as there should be no change of state.");
