...
public class NamespaceMapperJena extends StatementListener implements
    ...
    public NamespaceMapperJena(OntModel dataModel, OntModel metadataModel,
    ...
    private void rebuildNamespaceCache() {
        ...
        HashMap<String, List<String>> tempNamespaceToPrefixMap =
            new HashMap<String, List<String>>();
        metadataModel.enterCriticalSection(Lock.READ);
        int nsCount = 0;
        try {
            // Iterate through all the namespace objects
            ClosableIterator closeIt =
                metadataModel.listIndividuals(metadataModel.getResource(VitroVocabulary.NAMESPACE));
            try {
                for (Iterator namespaceIt = closeIt; namespaceIt.hasNext(); ) {
                    ...
                    RDFNode node = namespaceInd.getPropertyValue(
                        metadataModel.getProperty(VitroVocabulary.NAMESPACE_NAMESPACEURI));
                    ...
                    ClosableIterator closeIt2 = namespaceInd.listPropertyValues(
                        metadataModel.getProperty(VitroVocabulary.NAMESPACE_HASPREFIXMAPPING));
                    try {
                        for (Iterator namespacePrefixMappingIt = closeIt2;
                             namespacePrefixMappingIt.hasNext(); ) {
                            ...
                            if (value.canAs(Individual.class)) {
                                ...
                                RDFNode prefixValue = namespacePrefixMappingInd.getPropertyValue(
                                    metadataModel.getProperty(VitroVocabulary.NAMESPACE_PREFIX));
                                if ((prefixValue != null) && (prefixValue.isLiteral())) {
                                    String prefix = ((Literal) prefixValue).getLexicalForm();
                                    boolean isCurrent = true;
                                    RDFNode isCurrentValue = namespacePrefixMappingInd
                                        .getPropertyValue(metadataModel.getProperty(
                                            VitroVocabulary.NAMESPACE_ISCURRENTPREFIXMAPPING));
    ...
    private void createNewNamespaceObjects(List<String> newNamespaces) {
        for (String newNamespace : newNamespaces) {
            ...
            try {
                ...
                int id = 1;
                while (metadataModel.getIndividual(namespaceForNamespaceObjects + "ns" + id) !=
                    null) {
                    id++;
                }
                Individual nsInd = metadataModel
                    .createIndividual(namespaceForNamespaceObjects + "ns" + id,
                        metadataModel.getResource(VitroVocabulary.NAMESPACE));
                nsInd.addProperty(metadataModel.getProperty(VitroVocabulary.NAMESPACE_NAMESPACEURI),
                    newNamespace);
