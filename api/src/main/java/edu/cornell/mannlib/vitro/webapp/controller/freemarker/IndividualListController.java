...
public class IndividualListController extends FreemarkerHttpServlet {
    ...
    public static IndividualListResults getResultsForVClassIntersections(
        ...
        try {
            ...
            log.error(
                "Error retrieving individuals corresponding to intersection multiple classes." +
                    classUris.toString(), th);
            return IndividualListResults.EMPTY;
        }
    }

    private static IndividualListResults buildAndExecuteVClassQuery(List<String> classUris,
                                                                    int page, int pageSize,
                                                                    String alpha, VitroRequest vreq)
        throws SearchEngineException {
        ConfigurationProperties props = ConfigurationProperties.getBean(vreq);
        boolean languageFilter =
            Boolean.valueOf(props.getProperty(LANGUAGE_FILTER_PROPERTY, "false"));
        ...
        IndividualListResults indListResults =
            getResultsForVClassQuery(results, page, pageSize, alpha, vreq);
        return indListResults;
    }

    public static IndividualListResults getRandomResultsForVClass(String vclassURI, int page,
                                                                  int pageSize, VitroRequest vreq) {
        try {
            List<String> classUris = Collections.singletonList(vclassURI);
            IndividualListQueryResults results =
                buildAndExecuteRandomVClassQuery(classUris, page, pageSize,
                    vreq.getWebappDaoFactory().getIndividualDao());
    ...
    private static IndividualListResults getResultsForVClassQuery(
        IndividualListQueryResults results, int page, int pageSize, String alpha,
        VitroRequest vreq) {
        long hitCount = results.getHitCount();
        if (hitCount > pageSize) {
            return new IndividualListResults(hitCount, results.getIndividuals(), alpha, true,
                makePagesList(hitCount, pageSize, page, vreq));
        } else {
            return new IndividualListResults(hitCount, results.getIndividuals(), alpha, false,
    ...
    public static List<PageRecord> makePagesList(long size, int pageSize, int selectedPage,
        ...
        if (selectedPage < MAX_PAGES && requiredPages > MAX_PAGES) {
            ...
            for (int page = 1; page < requiredPages && page <= MAX_PAGES; page++) {
                records.add(
                    new PageRecord("page=" + page, Integer.toString(page), Integer.toString(page),
                        selectedPage == page));
            }
            records.add(new PageRecord("page=" + (MAX_PAGES + 1), Integer.toString(MAX_PAGES + 1),
                I18n.text(vreq, "paging_link_more"), false));
        } else if (requiredPages > MAX_PAGES && selectedPage + 1 > MAX_PAGES &&
            selectedPage < requiredPages - MAX_PAGES) {
            ...
            records.add(new PageRecord("page=" + (endPage + 1), Integer.toString(endPage + 1),
                I18n.text(vreq, "paging_link_more"), false));
            ...
            for (int i = 1; i <= requiredPages; i++) {
                records.add(new PageRecord("page=" + i, Integer.toString(i), Integer.toString(i),
                    selectedPage == i));
            }
        }
        return records;
    ...
    protected ResponseValues processRequest(VitroRequest vreq) {
        ...
        try {
            ...
            if (obj == null) { // look for vitroclass id parameter
                ...
                if (!StringUtils.isEmpty(vitroClassIdStr)) {
                    try {
                        ...
                        vclass = vreq.getWebappDaoFactory().getVClassDao()
                            .getVClassByURI(vitroClassIdStr);
                        if (vclass == null) {
                        ...
                        throw new HelpException(
                            "IndividualListController: url parameter 'vclassId' must be a URI string.");
                    }
                }
                ...
                throw new HelpException(
                    "IndividualListController: attribute 'vclass' must be of type "
            ...
            errorMessage =
                "Request attribute 'vclass' or request parameter 'vclassId' must be set before calling. Its value must be a class uri.";
        } catch (Throwable e) {
