...
public abstract class SparqlQueryApiExecutor {
     ...
     * types.
     *
     * @throws AcceptHeaderParsingException if the accept header was not in a valid format
     * @throws NotAcceptableException       if the accept header did not contain a content type that is
     *                                      supported by the query
     * @throws QueryParseException          if the query was not syntactically valid
     * @throws InvalidQueryTypeException    if the query was not SELECT, ASK, CONSTRUCT, or DESCRIBE
     */
