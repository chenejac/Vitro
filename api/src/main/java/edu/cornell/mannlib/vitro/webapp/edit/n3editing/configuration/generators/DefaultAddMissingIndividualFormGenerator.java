...
public class DefaultAddMissingIndividualFormGenerator implements EditConfigurationGenerator {
    private static final Log log =
        LogFactory.getLog(DefaultAddMissingIndividualFormGenerator.class);
    ...
    private void setEditKey(HttpSession session, EditConfigurationVTwo editConfiguration,
                            VitroRequest vreq) {
    ...
    private void initProcessParameters(VitroRequest vreq, HttpSession session,
        ...
        if (EditConfigurationUtils.isObjectProperty(predicateUri, vreq)) {
            ...
            log.error(
                "Add missing individual called for a data property instead of object property");
        }
    }
    ...
    private List<String> generateN3Required(VitroRequest vreq) {
        ...
        n3ForEdit.add(getN3PrefixesAsString() + "\n" + "?" + objectVarName + " rdf:type <" +
            getRangeClassUri(vreq) + "> . ");
        return n3ForEdit;
    }
    ...
    private String getRangeClassUri(VitroRequest vreq) {
        ...
        if (rangeClass == null) {
            ...
            if (rangeClass == null) {
                throw new Error(
                    "Cannot find class for range for property.  Looking for " +
                        prop.getRangeVClassURI());
            }
        }
    ...
    private boolean isEditOfExistingStatement(VitroRequest vreq) {
        ...
        if (EditConfigurationUtils
            .isDataProperty(EditConfigurationUtils.getPredicateUri(vreq), vreq)) {
            return false;
        }
