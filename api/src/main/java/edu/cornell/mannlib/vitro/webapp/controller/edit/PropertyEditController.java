...
public class PropertyEditController extends BaseEditController {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        ObjectPropertyDao propDaoLangNeut =
        ...
        ObjectProperty pLangNeut =
            propDaoLangNeut.getObjectPropertyByURI(request.getParameter("uri"));
        ...
        if (p.getParentURI() != null) {
            ...
            if (parent != null && parent.getURI() != null) {
                try {
                    parentPropertyStr = "<a href=\"propertyEdit?uri=" +
        ...
        if (p.getNamespace() != null) {
            Ontology ont = vreq.getUnfilteredWebappDaoFactory().getOntologyDao()
                .getOntologyByURI(p.getNamespace());
        ...
        results.add(
            p.getDomainPublic() == null ? "(no public label)" : p.getDomainPublic()); // column 6
        ...
        if (pLangNeut.getRangeVClassURI() != null) {
            ...
            if (rangeClass != null && rangeClass.getURI() != null &&
                rangeClass.getPickListName() != null) {
                try {
                    if (rangeClass.isAnonymous()) {
                        rangeStr = rangeClass.getPickListName();
                    } else {
                        rangeStr = "<a href=\"vclassEdit?uri=" +
        ...
        results.add("property: " + p.getDomainDisplayTier() + ", inverse: " +
            p.getRangeDisplayTier()); // column 18
        results.add("property: " + p.getDomainDisplayLimitInteger() + ", inverse: " +
            p.getRangeDisplayLimit());
        results.add(p.getCollateBySubclass() ? "true" : "false"); // column 19

        results.add(
            p.getCustomEntryForm() == null ? "(unspecified)" : p.getCustomEntryForm()); // column 20
        results.add(p.getSelectFromExisting() ? "true" : "false");   // column 21
        results.add(p.getOfferCreateNewOption() ? "true" : "false"); // column 22

        results.add(p.getDomainEntitySortDirection() == null ? "ascending" :
            p.getDomainEntitySortDirection()); // column 23
        ...
        ObjectPropertyDao opDao =
            vreq.getUnfilteredAssertionsWebappDaoFactory().getObjectPropertyDao();
        ...
        try {
            JSPPageHandler
                .renderBasicPage(request, response, "/templates/edit/specific/props_edit.jsp");
        } catch (Exception e) {
