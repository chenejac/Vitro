...
public class PropertyRestrictionBeanImpl extends PropertyRestrictionBean {
    ...
    private final Map<FullPropertyKey, PropertyRestrictionLevels> thresholdMap =
    ...
    public String toString() {
        ...
        for (FullPropertyKey key : keys) {
            buffer.append(key).append(" ").append(thresholdMap.get(key).getLevel(DISPLAY))
                .append(" ").append(thresholdMap.get(key).getLevel(MODIFY)).append(" ")
                .append(thresholdMap.get(key).getLevel(PUBLISH)).append("\n");
        }
