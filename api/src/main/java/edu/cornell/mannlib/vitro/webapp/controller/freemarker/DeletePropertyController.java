...
public class DeletePropertyController extends FreemarkerHttpServlet {
    ...
    private String handleErrors(VitroRequest vreq) {
        ...
        if (EditConfigurationUtils.isObjectProperty(predicateUri, vreq)) {
            ...
            if (prop == null) {
                return "In delete property controller, could not find object property " +
                    predicateUri;
            }
        } else {
            ...
            if (prop == null) {
                return "In delete property controller, could not find data property " +
                    predicateUri;
            }
        }
    ...
    private void logDataPropertyDeletionMessages(DataPropertyStatement dps) {
        log.debug(
            "attempting to delete dataPropertyStatement: subjectURI <" + dps.getIndividualURI() +
                ">");
    ...
    private void deleteObjectPropertyStatement(VitroRequest vreq) {
        ...
        wdf.getPropertyInstanceDao()
            .deleteObjectPropertyStatement(subjectUri, predicateUri, objectUri);
    }
    ...
    private Individual getObjectIndividualForDeletion(VitroRequest vreq, String objectUri) {
        ...
        if (object == null) {
            WebappDaoFactory wadf =
                ModelAccess.on(vreq.getSession().getServletContext()).getWebappDaoFactory();
    ...
    private void deleteObjectIndividual(VitroRequest vreq) {
        ...
        if (object != null) {
            log.warn("Deleting individual " + object.getName() +
                "since property has been set to force range object deletion or has been set to delete a specific object");
            WebappDaoFactory wdf = vreq.getWebappDaoFactory();
