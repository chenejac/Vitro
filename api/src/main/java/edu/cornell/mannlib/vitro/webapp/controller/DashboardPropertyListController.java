...
public class DashboardPropertyListController extends VitroHttpServlet {
    ...
    private static final Log log =
        LogFactory.getLog(DashboardPropertyListController.class.getName());
        ...
        try {
            ...
            if (obj == null || !(obj instanceof Individual)) {
                throw new HelpException(
                    "EntityMergedPropertyListController requires request.attribute 'entity' to be of"
            ...
            if (unassignedStr != null && unassignedStr.length() > 0) {
                ...
                log.debug("found temp group parameter \"" + unassignedStr +
            ...
            if (onlyPopulatedProps) {
                ...
                Collection<PropertyInstance> allPropInstColl =
                ...
                if (allPropInstColl != null) {
                    for (PropertyInstance pi : allPropInstColl) {
                        if (pi != null) {
                            ...
                            op.setLabel(
                                op.getDomainPublic()); // no longer relevant: pi.getSubjectSide() ? op.getDomainPublic() : op.getRangePublic());
                            mergedPropertyList.add(op);
                        } else {
                            log.error(
            ...
            if (mergedPropertyList != null) {
                ...
                if (groupedMode) {
                    ...
                    try {
                        groupsCount =
                            populateGroupsListWithProperties(pgDao, groupsList, mergedPropertyList,
                                groupForUngroupedProperties);
                    } catch (Exception ex) {
                        log.error("Exception on trying to populate groups list with properties: " +
                            ex.getMessage());
                        ex.printStackTrace();
                    }
                    try {
                                    ...
                                    for (Property p : g.getPropertyList()) {
                                        if (p instanceof ObjectProperty) {
                                            ...
                                            if (op.getObjectPropertyStatements() != null &&
                                                ...
                                                statementCount +=
                                                    op.getObjectPropertyStatements().size();
                                            }
                                        }
                                    }
                                }
    ...
    private int populateGroupsListWithProperties(PropertyGroupDao pgDao,
                                                 List<PropertyGroup> groupsList,
                                                 List<Property> mergedPropertyList,
        ...
        if (unassignedGroupName != null) {
            ...
            log.debug("creating temp property group " + unassignedGroupName +
                " for any unassigned properties");
        }
        switch (count) {
            case 0:
                log.warn(
                    "groupsList has no groups on entering populateGroupsListWithProperties(); will create a new group \"other\"");
                break;
            case 1:
                break;
            default:
                try {
                    ...
                    log.error(
                        "Exception on sorting groupsList in populateGroupsListWithProperties()");
                }
        }
        ...
        for (PropertyGroup pg : groupsList) {
            ...
            for (Property p : mergedPropertyList) {
                if (p.getURI() == null) {
                    ...
                    if (tempGroup !=
                        null) { // not assigned any group yet and are creating a group for unassigned properties
                        if (!alreadyOnPropertyList(tempGroup.getPropertyList(), p)) {
                            tempGroup.getPropertyList().add(p);
                            log.debug(
                                "adding property " + p.getLabel() + " to members of temp group " +
                                    unassignedGroupName);
                        }
            ...
            if (pg.getPropertyList().size() > 1) {
                try {
                    pg.getPropertyList()
                        .sort(new Property.DisplayComparatorIgnoringPropertyGroup());
                } catch (Exception ex) {
                    log.error(
                        "Exception sorting property group " + pg.getName() + " property list: " +
                            ex.getMessage());
                }
            }
        }
