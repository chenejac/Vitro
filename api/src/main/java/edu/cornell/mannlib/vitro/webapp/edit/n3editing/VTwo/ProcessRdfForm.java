...
//	private VitroRequest _vreq;
    ...
    public static AdditionsAndRetractions addDependentDeletes(AdditionsAndRetractions changes,
                                                              Model queryModel) {
        ...
        Model depResRetractions =
            DependentResourceDeleteJena
                .getDependentResourceDeleteForChange(changes.getAdditions(),
                    changes.getRetractions(), queryModel);
    ...
    public static void applyChangesToWriteModel(
        ...
        try {
            ...
            log.error(
                "error adding edit change n3required model to in memory model \n" + t.getMessage());
        } finally {
            if (writeModel instanceof OntModel) {
    ...
    public AdditionsAndRetractions process(
        ...
        if (configuration.isUpdate()) {
            changes = editExistingStatements(configuration, submission,
                vreq); //UQAM  vreq for getting linguistic context
        } else {
    ...
    protected void subInValuesToN3(
        ...
        List<String> requiredRetracts, List<String> optionalRetracts, VitroRequest vreq)
        ...
        substituteInForcedNewURIs(urisForNewResources, submission.getUrisFromForm(),
            requiredAsserts, optionalAsserts, URLToReturnTo);
        logSubstitue("Added form URIs that required new URIs", requiredAsserts, optionalAsserts,
            requiredRetracts, optionalRetracts);


        /* ********** Form submission URIs ********* */
        substituteInMultiURIs(submission.getUrisFromForm(), requiredAsserts, optionalAsserts,
            URLToReturnTo);
        logSubstitue("Added form URIs", requiredAsserts, optionalAsserts, requiredRetracts,
            optionalRetracts);
        ...
        for (String aKey : keys) {
            ...
            for (Literal aLiteral : literalFromForm) {
                if (aLiteral != null) {
                     ...
                     */
                    if (RDF.dtLangString.getURI().equals(aLiteratDT) &&
                        !aLiteral.getLanguage().isEmpty()) {
                        newLiteral = aLiteral;
                        ...
                        newLiteral =
                            ResourceFactory.createTypedLiteral(aText, aLiteral.getDatatype());
                    }
        ...
        substituteInMultiLiterals(literalsFromForm, requiredAsserts, optionalAsserts,
            URLToReturnTo);
        logSubstitue("Added form Literals", requiredAsserts, optionalAsserts, requiredRetracts,
            optionalRetracts);
        //Retractions does NOT get values from form.
        ...
        substituteInSubPredObjURIs(editConfig, requiredAsserts, optionalAsserts, requiredRetracts,
            optionalRetracts, URLToReturnTo);
        logSubstitue("Added sub, pred and obj URIs", requiredAsserts, optionalAsserts,
            requiredRetracts, optionalRetracts);
        ...
        logSubstitue("Added existing URIs", requiredAsserts, optionalAsserts, requiredRetracts,
            optionalRetracts);

        substituteInMultiLiterals(editConfig.getLiteralsInScope(),
            requiredAsserts, optionalAsserts, requiredRetracts, optionalRetracts, URLToReturnTo);
        logSubstitue("Added existing Literals", requiredAsserts, optionalAsserts, requiredRetracts,
            optionalRetracts);
        ...
        logSubstitue("Added URIs for new Resources", requiredAsserts, optionalAsserts,
            requiredRetracts, optionalRetracts);
    ...
    protected AdditionsAndRetractions parseN3ToChange(
        ...
        List<String> requiredDels, List<String> optionalDels, VitroRequest vreq,
        EditConfigurationVTwo editConfig) throws Exception {
        ...
        if (requiredDels != null && optionalDels != null) {
            ...
            try {
                ...
                if (!StringUtils.isBlank(editConfig.formUrl) &&
                    editConfig.formUrl.contains("RDFSLabelGenerator")) {
                ...
                if (editConfig != null && !StringUtils
                    .isBlank(editConfig.getLiteralsInScope().get("label").get(0).getLanguage()) &&
                    getLabelLanguage) {
    ...
    private void applyEditSubmissionPreprocessors(
        ...
        VitroRequest vreq) {
        List<EditSubmissionVTwoPreprocessor> preprocessors =
