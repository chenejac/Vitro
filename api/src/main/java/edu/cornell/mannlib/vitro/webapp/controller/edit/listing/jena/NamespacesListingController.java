    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ClosableIterator closeIt =
                ontModel.listIndividuals(ontModel.getResource(VitroVocabulary.NAMESPACE));
            try {
                for (Iterator namespaceIt = closeIt; namespaceIt.hasNext(); ) {
                    ...
                    String namespaceURI = "";
                    try {
                        namespaceURI = ((Literal) namespaceInd.getPropertyValue(namespaceURIProp))
                            .getLexicalForm();
                    } catch (Exception e) { /* ignore it for now */ }
                    results.add("XX");
                    results.add(namespaceURI);
                    RDFNode prefixMapping = namespaceInd.getPropertyValue(
                    ...
                    if ((prefixMapping != null) && (prefixMapping.canAs(Individual.class))) {
                        Individual prefixMappingInd =
                            (Individual) prefixMapping.as(Individual.class);
                        RDFNode prefixNode = prefixMappingInd.getPropertyValue(
                            ontModel.getProperty(VitroVocabulary.NAMESPACE_PREFIX));
                        if ((prefixNode != null) && prefixNode.isLiteral()) {
                            prefixFound = true;
                            try {
                                results.add(
                                    "<a href=\"editForm?controller=NamespacePrefix&amp;prefix=" +
                                        ((Literal) prefixNode).getLexicalForm() +
                                        "&amp;namespace=" +
                                        URLEncoder.encode(namespaceURI, "UTF-8") + "\">" +
                                        ((Literal) prefixNode).getLexicalForm() + "</a>");
                            } catch (Exception e) {
                                //
                            }
                        }
                    ...
                    if (!prefixFound) {
                        try {
                            results.add(
                                "<a href=\"editForm?controller=NamespacePrefix&amp;namespace=" +
                                    URLEncoder.encode(namespaceURI, "UTF-8") + "\">add prefix</a>");
                        } catch (Exception e) {
                            //
                        }
                    }
