...
public class ExtendedRdfAssembler {
    ...
    private static final Property extendedLinkedDataProperty =
        ResourceFactory.createProperty(namespaces.get("vitro") + "extendedLinkedData");
    private static final Literal xsdTrue =
    ...
    public static boolean includeInLinkedData(Resource object, Model contextModel) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (stmt.getObject().isResource() && contextModel
                    .contains(stmt.getObject().asResource(), extendedLinkedDataProperty, xsdTrue)) {
                    retval = true;
                    break;
                }
            }
    ...
    public ResponseValues assembleRdf() {
        ...
        Model newModel =
            getRDF(individual, ontModel, ModelFactory.createDefaultModel(), 0, includes);
    ...
    private Model getRDF(Individual entity, OntModel contextModel, Model newModel, int recurseDepth,
        ...
        for (DataPropertyStatement ds : dstates) {
            ...
            if ((ds.getLanguage()) != null && (ds.getLanguage().length() > 0)) {
                ...
                lit = newModel.createTypedLiteral(ds.getData(),
                    typeMapper.getSafeTypeByName(ds.getDatatypeURI()));
            } else {
        ...
        if (recurseDepth < 5) {
            ...
            for (ObjectPropertyStatement os : ostates) {
                ...
                if (includeInLinkedData(obj, contextModel)) {
                    newModel.add(
                        getRDF(os.getObject(), contextModel, newModel, recurseDepth + 1, includes));
                } else {
                    contextModel.enterCriticalSection(Lock.READ);
                    try {
...
//		}
        ...
        if (recurseDepth == 0 && includes != null && entity.isVClass(PERSON_CLASS_URI)) {
            ...
            for (String include : includes) {
                ...
                Model extendedModel = ExtendedLinkedDataUtils
                    .createModelFromQueries(ctx, rootDir, contextModel, entity.getURI());
    ...
    private Model getLabelAndTypes(Individual entity, Model ontModel, Model newModel) {
        for (VClass vclass : entity.getVClasses()) {
            newModel.add(newModel.getResource(entity.getURI()), RDF.type,
                newModel.getResource(vclass.getURI()));
        }

        ontModel.enterCriticalSection(Lock.READ);
        try {
            newModel.add(ontModel
                .listStatements(ontModel.getResource(entity.getURI()), RDFS.label, (RDFNode) null));
        } finally {
     ...
     */
    private Model getStatementsWithUntypedProperties(Resource subject, OntModel contextModel,
                                                     OntModel assertionsModel, Model newModel) {
        contextModel.enterCriticalSection(Lock.READ);
        try {
            StmtIterator iter =
                contextModel.listStatements(subject, (Property) null, (RDFNode) null);
            while (iter.hasNext()) {
                Statement stmt = iter.next();
