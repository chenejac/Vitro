...
public class DeleteIndividualController extends FreemarkerHttpServlet {
    ...
    private static String getDeleteQueryForTypes(List<String> types, Model displayModel) {
        ...
        for (String type : types) {
            ...
            try {
                QueryExecution qexec =
                    QueryExecutionFactory.create(queryForTypeSpecificDeleteQuery, displayModel,
                        initialBindings);
                try {
        ...
        if (!foundType.isEmpty()) {
            ...
            if (!deleteQueryText.contains(INDIVIDUAL_URI)) {
                log.error(
                    "Safety check failed. Delete query text should contain " + INDIVIDUAL_URI + ", "
            ...
            log.debug(
                "For most specific types: " + types.stream().collect(Collectors.joining(",")) +
                    " no delete query was found. Using default query \n" + deleteQueryText);
        }
    ...
    private List<String> getObjectMostSpecificTypes(String individualUri, VitroRequest vreq) {
        ...
        Individual individual =
            vreq.getWebappDaoFactory().getIndividualDao().getIndividualByURI(individualUri);
        if (individual != null) {
    ...
    private Model getIndividualsToDelete(String targetIndividual, String deleteQuery,
                                         VitroRequest vreq) {
        try {
            ...
            QueryExecution qexec =
                QueryExecutionFactory.create(queryForTypeSpecificDeleteQuery, ontModel, bindings);
    ...
    private void deleteIndividuals(Model model, VitroRequest vreq) {
        ...
        try {
            ...
            cs.addRemoval(in, RDFServiceUtils.getSerializationFormatFromJenaString("N3"),
