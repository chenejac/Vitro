...
public class Formatter {
    public static String format(SearchInputDocument doc) {
        ...
        sb.append("SearchInputDocument: Name='").append(getValueFromField(doc, "nameRaw"))
            .append("', URI='").append(getValueFromField(doc, "URI")).append("', boost='")
            .append(doc.getDocumentBoost()).append("', ").append(doc.getFieldMap().size())
            .append(" fields \n");
    ...
    private static StringBuilder format(SearchInputField field, String padding) {
        StringBuilder sb = new StringBuilder();
        sb.append(padding).append("SearchInputField: Name='").append(field.getName())
            .append("', boost='").append(field.getBoost()).append("', ")
            .append(field.getValues().size()).append(" values \n");
    ...
    public static String format(SearchResponse response) {
        StringBuilder sb = new StringBuilder();
        sb.append("SearchResponse: ").append(response.getResults().size()).append(" results, ")
            .append(response.getFacetFields().size()).append(" facet fields, ")
            .append(response.getHighlighting().size()).append(" highlights\n");
    ...
    private static String format(SearchResultDocument doc, String padding) {
        ...
        sb.append("SearchResultDocument: Name='").append(getValueFromField(doc, "nameRaw"))
            .append("', URI='").append(getValueFromField(doc, "URI")).append("', ")
            .append(doc.getFieldNames().size()).append(" fields with ").append(valuesCount)
            .append(" values \n");

        for (String fieldName : new TreeMap<>(doc.getFieldValuesMap()).keySet()) {
            sb.append(padding).append("Field: Name='").append(fieldName).append("', ")
                .append(doc.getFieldValues(fieldName).size()).append(" values \n");
