...
public class IndividualsForClassesDataGetter extends DataGetterBase implements DataGetter {
     ...
     */
    public IndividualsForClassesDataGetter(VitroRequest vreq, Model displayModel,
                                           String dataGetterURI) {
    ...
    protected void configure(VitroRequest vreq, Model displayModel, String dataGetterURI) {
        ...
        this.classGroupURI =
            DataGetterUtils.getClassGroupForDataGetter(displayModel, dataGetterURI);
    ...
    protected Map<String, Object> getClassIntersectionsMap(Model displayModel) {
        ...
        try {
            QueryExecution qexec =
                QueryExecutionFactory.create(dataGetterQuery, displayModel, initBindings);
    ...
    protected void processClassesAndRestrictions(VitroRequest vreq, ServletContext context,
                                                 HashMap<String, Object> data, List<String> classes,
    ...
    private void processIntersections(VitroRequest vreq,
        ...
        if (restrictClasses.size() > 0) {
            ...
            for (VClass v : vclassList) {
                ...
                if (oldCount != count) {
                    log.debug(
                        "Old count was " + v.getEntityCount() + " and New count for " + v.getURI() +
                            " is " + count);
    ...
    private int retrieveCount(VitroRequest vreq, ServletContext context, VClass v,
                              List<VClass> restrictClasses) {
    ...
    private void processClassesForDisplay(VitroRequest vreq, ServletContext context,
                                          HashMap<String, Object> data, List<String> classes) {
        ...
        for (String classUri : classes) {
            ...
            if (vclass != null) {
                log.debug("VClass does exist for " + classUri + " and entity count is " +
                    vclass.getEntityCount());
                vClasses.add(vclass);
            } else {
    ...
    private void processRestrictionClasses(VitroRequest vreq,
        ...
        try {
            ...
            log.debug("Variable name for including restriction classes " +
                getRestrictClassesTemplateName());
    ...
    public String getType() {
        return DataGetterUtils
            .generateDataGetterTypeURI(IndividualsForClassesDataGetter.class.getName());
    }
