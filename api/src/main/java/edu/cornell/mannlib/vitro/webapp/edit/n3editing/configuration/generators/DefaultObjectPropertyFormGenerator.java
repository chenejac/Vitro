...
public class DefaultObjectPropertyFormGenerator implements EditConfigurationGenerator {
    ...
    public EditConfigurationVTwo getEditConfiguration(VitroRequest vreq,
        ...
        if (!EditConfigurationUtils
            ...
            throw new Exception(
                "DefaultObjectPropertyFormGenerator does not handle data properties.");
        }
        ...
        if (DefaultAddMissingIndividualFormGenerator.isCreateNewIndividual(vreq, session)) {
            EditConfigurationGenerator generator = JspToGeneratorMapping
                .createFor("defaultAddMissingIndividualForm.jsp",
                    DefaultAddMissingIndividualFormGenerator.class);
...
//        WebappDaoFactory ctxDaoFact = vreq.getLanguageNeutralWebappDaoFactory();
        ...
        WebappDaoFactory ctxDaoFact = ModelAccess.on(vreq)
            .getWebappDaoFactory(LanguageOption.LANGUAGE_AWARE, PolicyOption.POLICY_NEUTRAL);
        ...
        if (subject != null) {
            ...
            for (VClass vclass : vClasses) {
                List<VClass> rangeVclasses =
                    wDaoFact.getVClassDao().getVClassesForProperty(vclass.getURI(), predicateUri);
    ...
    private boolean tooManyRangeOptions(VitroRequest vreq, HttpSession session)
    ...
    private EditConfigurationVTwo getDefaultObjectEditConfiguration(VitroRequest vreq,
                                                                    HttpSession session)
        throws Exception {
    ...
    private EditConfigurationVTwo getCustomErrorEditConfiguration(VitroRequest vreq,
                                                                  HttpSession session) {
        EditConfigurationVTwo editConfiguration = new EditConfigurationVTwo();
    ...
    private boolean isSelectFromExisting(VitroRequest vreq) {
        ...
        ObjectProperty objProp = EditConfigurationUtils
            .getObjectPropertyForPredicate(vreq, EditConfigurationUtils.getPredicateUri(vreq));
    ...
    private void processProhibitedFromSearch(VitroRequest vreq, HttpSession session,
        ...
        if (isSelectFromExisting(vreq)) {
            ...
            OntModel displayOntModel =
                ModelAccess.on(session.getServletContext()).getOntModel(DISPLAY);
    ...
    public void addFormSpecificDataForAC(EditConfigurationVTwo editConfiguration, VitroRequest vreq,
                                         HttpSession session) throws SearchEngineException {
        HashMap<String, Object> formSpecificData = new HashMap<String, Object>();
        //Get the edit mode
