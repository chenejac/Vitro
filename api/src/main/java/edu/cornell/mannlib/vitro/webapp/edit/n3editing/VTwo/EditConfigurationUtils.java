...
public class EditConfigurationUtils {
    ...
    public static boolean isObjectProperty(String predicateUri, VitroRequest vreq) {
        ...
        log.debug("For " + predicateUri + ", object property from dao null? " + (op == null) +
            " and data property  null?" + (dp == null));
     ...
     */
    public static DataPropertyStatement getDataPropertyStatement(VitroRequest vreq,
                                                                 HttpSession session,
                                                                 Integer dataHash,
                                                                 String predicateUri) {
        ...
        if (dataHash != 0) {
            ...
            dps = RdfLiteralHash
                .getPropertyStmtByHash(EditConfigurationUtils.getSubjectUri(vreq), predicateUri,
                    dataHash, model);
        }
        return dps;
    }
    ...
    public static Integer getDataHash(VitroRequest vreq) {
        ...
        if (datapropKey != null && datapropKey.trim().length() > 0) {
            try {
                ...
                log.error(
                    "Cannot decode incoming dataprop key str " + datapropKey + "as integer hash");
    ...
    public static Map<String, List<String>> getExistingLiteralValues(VitroRequest vreq,
                                                                     EditConfigurationVTwo editConfig) {
        Map<String, List<String>> literalsInScopeStringValues =
            transformLiteralMap(editConfig.getLiteralsInScope());
    ...
    public static String generateHTMLForElement(VitroRequest vreq, String fieldName,
                                                EditConfigurationVTwo editConfig) {
        String html = "";
        ...
        MultiValueEditSubmission editSub =
            EditSubmissionUtils.getEditSubmissionFromSession(vreq.getSession(), editConfig);
    ...
    public static EditConfigurationVTwo getEditConfiguration(HttpServletRequest request) {
        ...
        EditConfigurationVTwo editConfiguration =
            EditConfigurationVTwo.getConfigFromSession(session, request);
