...
public class ListingControllerWebUtils {
    ...
    public static synchronized String formatIndividualLink(Individual ind) {
        try {
            ...
            return "<a href=\"entityEdit?uri=" + URLEncoder.encode(ind.getURI(), "UTF-8") + "\">" +
                nameStr + "</a>";
        ...
        if (vList != null) {
            ...
            for (Object obj : vList) {
                try {
                    ...
                    try {
                        linksStr.append("<a href=\"vclassEdit?uri=")
                            .append(URLEncoder.encode(vclass.getURI(), "UTF-8")).append("\">")
                            .append(vclass.getName()).append("</a>");
                    } catch (UnsupportedEncodingException e) {
                    ...
                    if (obj == null) {
                        log.error(ListingControllerWebUtils.class.getName() +
                            " could not format null VClass");
                    }
                }
            }
        }
