...
public class PageDaoJena extends JenaBaseDao implements PageDao {
    ...
    static final protected String pageQueryString =
        prefixes + "\n" +
            ...
            "    OPTIONAL { ?pageUri <" + DisplayVocabulary.REQUIRES_BODY_TEMPLATE +
            "> ?bodyTemplate }.\n" +
    ...
    static final protected String individualsForClassesDataGetterQueryString =
        prefixes + "\n" +
            ...
            "    OPTIONAL {?dg <" + DisplayVocabulary.RESTRICT_RESULTS_BY +
            "> ?restrictClass } .\n" +
            "} \n";
    ...
    static final protected String institutionalInternalClassQueryString =
        prefixes + "\n" +
            ...
            "    ?dg <" + DisplayVocabulary.RESTRICT_RESULTS_BY_INTERNAL +
            "> ?restrictsByInternalClass .\n" +
            "} \n";
    ...
    static final protected String individualsForClassesInternalQueryString =
        prefixes + "\n" +
            ...
            " ?dg <" + DisplayVocabulary.GETINDIVIDUALS_FOR_CLASS + "> ?class . \n" +
            " OPTIONAL {  ?dg <" + DisplayVocabulary.RESTRICT_RESULTS_BY_INTERNAL +
            "> ?isInternal } .\n" +
            "} \n";
    ...
    static {
        ...
        try {
            individualsForClassesQuery =
                QueryFactory.create(individualsForClassesDataGetterQueryString);
        } catch (Throwable th) {
            log.error(
                "could not create SPARQL query for individualsForClassesQuery " + th.getMessage());
            log.error(individualsForClassesDataGetterQueryString);
        }

        try {
            individualsForClassesRestrictedQuery =
                QueryFactory.create(individualsForClassesRestrictedQueryString);
        } catch (Throwable th) {
            log.error("could not create SPARQL query for individualsForClassesRestrictedQuery " +
                th.getMessage());
            log.error(individualsForClassesDataGetterQueryString);
        }
        ...
        try {
            institutionalInternalClassQuery =
                QueryFactory.create(institutionalInternalClassQueryString);
        } catch (Throwable th) {
            log.error("could not create SPARQL query for institutionalInternalClassQuery " +
                th.getMessage());
        ...
        try {
            ...
            log.error("could not create SPARQL query for individualsForClassesInternalQuery " +
                th.getMessage());
    ...
    public Map<String, Object> getPage(String pageUri) {
        ...
        try {
            QueryExecution qexec =
                QueryExecutionFactory.create(pageQuery, displayModel, initialBindings);
            try {
        ...
        try {
            QueryExecution qexec = QueryExecutionFactory
                .create(pageDataGetterTypeQuery, displayModel, initialBindings);
            try {
        ...
        try {
            QueryExecution qexec =
                QueryExecutionFactory.create(dataGetterURIsQuery, displayModel, initialBindings);
            try {
                ResultSet rs = qexec.execSelect();
                while (rs.hasNext()) {
                    QuerySolution soln = rs.next();
    ...
    public String getClassGroupPage(String pageUri) {
        ...
        try {
            QueryExecution qexec =
                QueryExecutionFactory.create(classGroupPageQuery, displayModel, initialBindings);
            try {
                ...
                if (classGroupsForPage.size() > 1) {
                    log.error(
                        "More than one display:forClassGroup defined in display model for page " +
                            pageUri);
                }
    ...
    public Map<String, Object> getClassesAndRestrictionsForPage(String pageUri) {
        ...
        try {
            QueryExecution qexec = QueryExecutionFactory
                .create(individualsForClassesQuery, displayModel, initialBindings);
            try {
                ...
                while (resultSet.hasNext()) {
                    ...
                    if (!restrictClass.isEmpty() &&
                        !restrictClassesPresentMap.containsKey(restrictClass)) {
    ...
    public List<String> getDataGetterClass(String pageUri) {
        ...
        try {
            QueryExecution qexec =
                QueryExecutionFactory.create(dataGetterClassQuery, displayModel, initialBindings);
            try {
                ResultSet resultSet = qexec.execSelect();
                while (resultSet.hasNext()) {
                    QuerySolution soln = resultSet.next();
