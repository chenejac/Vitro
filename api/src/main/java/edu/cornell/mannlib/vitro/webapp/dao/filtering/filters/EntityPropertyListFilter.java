...
public class EntityPropertyListFilter extends UnaryFunctor<List<Property>, List<Property>> {
    ...
    private static final org.apache.jena.rdf.model.Property MASKS_PROPERTY =
        ResourceFactory.createProperty(VitroVocabulary.MASKS_PROPERTY);
    ...
    public EntityPropertyListFilter(OntModel ontModel) {
        ...
        try {
            StmtIterator maskStmtIt =
                ontModel.listStatements((Resource) null, MASKS_PROPERTY, (RDFNode) null);
            while (maskStmtIt.hasNext()) {
                Statement maskStmt = maskStmtIt.nextStatement();
                if (!maskStmt.getSubject().isAnon() && maskStmt.getObject().isResource() &&
                    !((Resource) maskStmt.getObject()).isAnon()) {
