...
public class PrimitiveRdfEdit extends VitroAjaxController {
    ...
    protected void doRequest(VitroRequest vreq,
        ...
        if (!("N-TRIPLE".equals(format) || "TURTLE".equals(format) || "TTL".equals(format)
            || "N3".equals(format) || "RDF/XML-ABBREV".equals(format) ||
            "RDF/XML".equals(format))) {
        ...
        try {
            ...
            doError(response,
                "Error reading RDF, set log level to debug for this class to get error messages in the server logs.",
                SC_BAD_REQUEST);
            return;
        }
    ...
    Set<Model> parseRdfParam(String[] parameters, String format) throws Exception {
        ...
        for (String param : parameters) {
            try {
                ...
                throw new Exception(
                    "Error reading RDF, set log level to debug for this class to get error messages in the sever logs.");
            }
        }
        return models;
    }
