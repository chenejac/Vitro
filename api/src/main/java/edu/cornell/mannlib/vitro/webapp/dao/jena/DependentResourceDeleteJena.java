...
public class DependentResourceDeleteJena {
    ...
    public static List<Statement> getDependentResourceDeleteList(Resource deleteMe,
                                                                 Model sourceModel) {
        List<Statement> deletes = new LinkedList<Statement>();
        for (Statement stmt : getAllStatements(deleteMe, sourceModel)) {
            deletes.add(stmt);
            deletes.addAll(
                getDependentResourceDeleteList(stmt, sourceModel, new HashSet<String>(), false));
        }
        return deletes;
    }

    public static Model getDependentResourceDeleteForChange(Model assertions, Model retractions,
                                                            Model sourceModel) {
    ...
    private static List<Statement> getDependentResourceDeleteList(Statement stmt, Model model,
                                                                  Set<String> visitedUris,
                                                                  boolean perviousWasDependentResource) {
        if (stmt == null) {
        ...
        if ((obj.canAs(Resource.class) && isPredicateDependencyRelation(stmt.getPredicate(), model))
            ...
            if (!visitedUris.contains(id)) {
                ...
                for (Statement stubStmt : getAllStatements(res, model)) {
                    toDelete
                        .addAll(getDependentResourceDeleteList(stubStmt, model, visitedUris, true));
                }
            }
        }
        return toDelete;
