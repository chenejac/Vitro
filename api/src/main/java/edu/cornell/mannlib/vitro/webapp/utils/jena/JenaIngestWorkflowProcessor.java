...
public class JenaIngestWorkflowProcessor {
    ...
    public List<Individual> getWorkflowSteps(Individual startingWorkflowStep) {
        ...
        Individual currentInd = (startingWorkflowStep == null) ?
            getWorkflowStep(workflowInd.getPropertyValue(WorkflowOntology.firstStep)) :
    ...
    private Model getModel(RDFNode modelNode) {
        ...
        String modelNameStr =
            ((Literal) modelInd.getPropertyValue(WorkflowOntology.modelName).as(Literal.class))
                .getLexicalForm();
    ...
    private class ClearModelAction implements ActionHandler {
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.ClearModelAction)) {
                Model sourceModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.sourceModel));
        ...
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.AddModelAction)) {
                Model sourceModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.sourceModel));
                Model modelToAdd =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.modelToAdd));
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                Boolean applyChangesDirectlyToSource = false;
                RDFNode valueNode =
                    actionInd.getPropertyValue(WorkflowOntology.applyChangesDirectlyToSource);
                if ((valueNode != null) && (valueNode.isLiteral())) {
                    applyChangesDirectlyToSource =
                        ((Literal) valueNode.as(Literal.class)).getBoolean();
                }
    ...
    private class SubtractModelsAction implements ActionHandler {
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.SubtractModelAction)) {
                Model sourceModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.sourceModel));
                Model modelToSubtract =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.modelToSubtract));
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                Boolean applyChangesDirectlyToSource = false;
                RDFNode valueNode =
    ...
    private class ExecuteSparqlConstructAction implements ActionHandler {
        ...
        private static final String QUERY_STR_PROPERTY =
        ...
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.SPARQLCONSTRUCTAction)) {
                ...
                for (RDFNode node : (List<RDFNode>) actionInd
                    .listPropertyValues(WorkflowOntology.sourceModel).toList()) {
                ...
                if (actionInd.getPropertyValue(WorkflowOntology.destinationModel) == null) {
                    log.debug(
                        "Error: destination model for SPARQL Construct action not specified for this action");
                    return null;
                }
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                Model tempModel = ModelFactory.createDefaultModel();
                OntResource sparqlQuery =
                    (OntResource) actionInd.getPropertyValue(WorkflowOntology.sparqlQuery);
                String queryStr = ((Literal) sparqlQuery
                    .getPropertyValue(ResourceFactory.createProperty(QUERY_STR_PROPERTY)))
                    .getLexicalForm();
    ...
    private class SmushResourcesAction implements ActionHandler {
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.SmushResourcesAction)) {
                ...
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                String smushPropertyURI =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.smushOnProperty))
                        .getLexicalForm();
                destinationModel.enterCriticalSection(Lock.WRITE);
                try {
                    destinationModel.add(utils.smushResources(sourceModel,
                        ResourceFactory.createProperty(smushPropertyURI)));
                } finally {
                    destinationModel.leaveCriticalSection();
                }
                return new ActionResultImpl();
    ...
    private class NameBlankNodesAction implements ActionHandler {
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.NameBlankNodesAction)) {
                ...
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                String uriPrefix = getValue(actionInd.getPropertyValue(WorkflowOntology.uriPrefix))
                    .getLexicalForm();
        ...
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.SplitPropertyValuesAction)) {
                ...
                Model destinationModel =
                    getModel(actionInd.getPropertyValue(WorkflowOntology.destinationModel));
                String propertyURI =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.originalProperty))
                        .getLexicalForm();
                String newPropertyURI =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.newProperty))
                        .getLexicalForm();
                String splitRegex =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.splitRegex))
                        .getLexicalForm();
                boolean trim = true;
                try {
                ...
                destinationModel.enterCriticalSection(Lock.WRITE);
                try {
                    destinationModel.add(utils
                        .splitPropertyValues(sourceModel, propertyURI, splitRegex, newPropertyURI,
                            trim));
                } finally {
                    destinationModel.leaveCriticalSection();
                }
    ...
    private class ProcessPropertyValueStringsAction implements ActionHandler {
        public ActionResult handleAction(Individual actionInd) {
            if (instanceOf(actionInd, WorkflowOntology.ProcessPropertyValueStringsAction)) {
                ...
                try {
                    additionsModel =
                        getModel(actionInd.getPropertyValue(WorkflowOntology.additionsModel));
                } catch (Exception e) {
                }
                ...
                String processorClass =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.processorClass))
                        .getLexicalForm();
                String processorMethod =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.processorMethod))
                        .getLexicalForm();
                String propertyURI =
                    getValue(actionInd.getPropertyValue(WorkflowOntology.originalProperty))
                        .getLexicalForm();
                ...
                try {
                    ...
                    utils.processPropertyValueStrings(sourceModel, destinationModel, additionsModel,
                        retractionsModel, processorClass, processorMethod, propertyURI,
                        newPropertyURI);
                } finally {
                    destinationModel.leaveCriticalSection();
                }
