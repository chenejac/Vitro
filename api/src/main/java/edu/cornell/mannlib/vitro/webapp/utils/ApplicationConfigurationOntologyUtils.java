...
public class ApplicationConfigurationOntologyUtils {
    ...
    public static List<FauxObjectPropertyWrapper> getPopulatedFauxOPs(
        List<ObjectProperty> populatedObjectProperties, Individual subject, VitroRequest vreq) {
    ...
    public static List<FauxDataPropertyWrapper> getPopulatedFauxDPs(
        List<DataProperty> populatedDataProperties, Individual subject, VitroRequest vreq) {
    ...
    private static List<FauxObjectPropertyWrapper> getPopulatedFauxObjectProperties(
        ObjectProperty op, Individual subject, VitroRequest vreq) {
        ...
        WebappDaoFactory wadf =
            new WebappDaoFactoryJena(ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM, union));
        ...
        List<FauxObjectPropertyWrapper> fauxObjectProps =
    ...
    private static List<FauxDataPropertyWrapper> getPopulatedFauxDataProperties(DataProperty dp,
                                                                                Individual subject,
                                                                                VitroRequest vreq) {
        Model displayModel = vreq.getDisplayModel();
        Model tboxModel = vreq.getOntModelSelector().getTBoxModel();
    ...
    public static List<Property> getPossibleFauxProps(List<? extends FauxPropertyWrapper> curProps,
                                                      Individual subject, VitroRequest vreq,
                                                      boolean isData) {
        Model displayModel = vreq.getDisplayModel();
        Model tboxModel = vreq.getOntModelSelector().getTBoxModel();
        Model union = ModelFactory.createUnion(displayModel, tboxModel);
        Map<String, FauxProperty> curPropsMap = curProps.stream()
            .collect(Collectors
                .toMap(FauxPropertyWrapper::getContextUri, FauxPropertyWrapper::getFauxProperty));
        WebappDaoFactory wadf =
            new WebappDaoFactoryJena(ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM, union));
        FauxPropertyDao fpDao = wadf.getFauxPropertyDao();
        ...
        try {
            ...
            while (rs.hasNext()) {
                ...
                String contextURI = qsoln.getResource("context").getURI();
                if (isDomainMatchSubject(domainURI, subject) &&
                    !curPropsMap.containsKey(contextURI)) {
                    if (isData) {
    ...
    private static void addObjectProperty(List<Property> fauxProps, String basePropertyUri,
                                          String contextURI,
    ...
    private static void addDataProperty(List<Property> fauxProps, String basePropertyUri,
                                        String contextURI,
