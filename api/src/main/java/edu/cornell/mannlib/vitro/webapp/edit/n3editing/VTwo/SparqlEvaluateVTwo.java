...
public class SparqlEvaluateVTwo {
    ...
    public void evaluateForAdditionalUris(EditConfigurationVTwo editConfig) {
        ...
        Map<String, List<String>> varsToUris =
            sparqlEvaluateForUris(editConfig, editConfig.getSparqlForAdditionalUrisInScope());
    ...
    public void evalulateForAdditionalLiterals(EditConfigurationVTwo editConfig) {
        ...
        Map<String, List<Literal>> varsToLiterals = sparqlEvaluateForLiterals(editConfig,
    ...
    public void evaluateForExistingUris(EditConfigurationVTwo editConfig) {
        ...
        Map<String, List<String>> varsToUris =
            sparqlEvaluateForUris(editConfig, editConfig.getSparqlForExistingUris());
        editConfig.getUrisInScope().putAll(varsToUris);
    }
    ...
    public void evaluateForExistingLiterals(EditConfigurationVTwo editConfig) {
        ...
        Map<String, List<Literal>> varsToLiterals =
            sparqlEvaluateForLiterals(editConfig, editConfig.getSparqlForExistingLiterals());
        editConfig.getLiteralsInScope().putAll(varsToLiterals);
    }
...
//    }
    ...
    public Map<String, List<Literal>> sparqlEvaluateForLiterals(EditConfigurationVTwo editConfig,
        ...
        for (String var : varToSparql.keySet()) {
            ...
            if (query == null || EditConfigurationVTwo.USE_SYSTEM_VALUE.equals(query)) {
                log.debug(
                    "Query is null or using system value so will not continue with rest of method");
                continue;
            }
            List<String> queryStrings = new ArrayList<String>();
            queryStrings.add(query);
            ...
            varToLiterals
                .put(var, queryToLiteral(queryStrings.get(0)));   //might result in (key -> null)
        }

        return varToLiterals;
    }

    public Map<String, List<String>> sparqlEvaluateForUris(EditConfigurationVTwo editConfig,
                                                           Map<String, String> varToSparql) {
        Map<String, List<String>> uriScope = editConfig.getUrisInScope();
        Map<String, List<Literal>> literalScope = editConfig.getLiteralsInScope();
...
//    }
    ...
    public List<String> queryToUri(String querystr) {
        ...
        try {
            ...
            throw new Error(
                "could not parse SPARQL in queryToUri: \n" + querystr + '\n' + ex.getMessage());
        } finally {
            if (qe != null) {
                qe.close();
            }
        ...
        if (log.isDebugEnabled()) {
            log.debug("queryToUri() query: '" + querystr + "'\nvalue: '" + values.toString() + "'");
        }
        return values;
    }
    ...
    public List<Literal> queryToLiteral(String querystr) {
        ...
        try {
            ...
            throw new Error(
                "could not parse SPARQL in queryToLiteral: \n" + querystr + '\n' + ex.getMessage());
        } finally {
            if (qe != null) {
                qe.close();
            }
