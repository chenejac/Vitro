...
public abstract class RDFServiceJena extends RDFServiceImpl implements RDFService {
    ...
    private void removeBlankNodesWithSparqlUpdate(Dataset dataset, Model model, String graphURI) {
        ...
        if (blankNodeModel.size() == 1) {
            ...
            log.debug(
                "This could result in the deletion of multiple triples if multiple blank nodes match the same triple pattern.");
        }
    ...
    private void getRDFModel(String query, boolean construct, Model model)
        throws RDFServiceException {
        DatasetWrapper dw = getDatasetWrapper();
        try {
            Dataset d = dw.getDataset();
    ...
    public long countTriples(RDFNode subject, RDFNode predicate, RDFNode object)
        ...
        Query countQuery = QueryFactory
            .create("SELECT (COUNT(?s) AS ?count) WHERE { ?s ?p ?o } ORDER BY ?s ?p ?o",
