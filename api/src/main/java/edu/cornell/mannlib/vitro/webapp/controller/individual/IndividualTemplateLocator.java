...
class IndividualTemplateLocator {
    ...
    String findTemplate() {
        ...
        if (individual.getVClass() != null) {
            ...
            for (VClass vclass : directClasses) {
                ...
                if (customTemplate != null) {
                    if (customTemplate.length() > 0) {
                        vclassName = vclass
                            .getName(); // reset entity vclassname to name of class where a custom view; this call has side-effects
                        log.debug(
                            "Found direct class [" + vclass.getName() + "] with custom view " +
                                customTemplate + "; resetting entity vclassName to this class");
                        break;
                    } else {
                        customTemplate = null;
                    }
                ...
                for (VClass vclass : inferredClasses) {
                    ...
                    if (customTemplate != null) {
                        if (customTemplate.length() > 0) {
                            ...
                            log.debug("Found inferred class [" + vclass.getName() +
                                "] with custom view " + customTemplate);
                            break;
                        } else {
                            customTemplate = null;
                        }
            ...
            if (customTemplate == null && simpleReasoner != null &&
                simpleReasoner.isABoxReasoningAsynchronous()) {
                log.debug(
                    "Checking superclasses for custom template specification because ABox reasoning is asynchronous");
                    ...
                    for (String uri : superClassUris) {
                        ...
                        if (customTemplate != null) {
                            if (customTemplate.length() > 0) {
                                ...
                                log.debug("Found superclass [" + vclass.getName() +
                                    "] with custom view " + customTemplate);
                                break;
                            } else {
                                customTemplate = null;
