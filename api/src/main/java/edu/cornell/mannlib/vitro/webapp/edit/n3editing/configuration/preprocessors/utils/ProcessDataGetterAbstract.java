...
public abstract class ProcessDataGetterAbstract implements ProcessDataGetterN3 {
    ...
    //For existing values
    protected Map<String, List<Literal>> existingLiteralValues =
        new HashMap<String, List<Literal>>();
    ...
    public void populateExistingDataGetterURI(String dataGetterURI, int counter) {
        existingUriValues.put(this.getVarName("dataGetter", counter),
            new ArrayList<String>(Arrays.asList(dataGetterURI)));

    }

    public void populateExistingClassType(String classType, int counter) {
        existingUriValues.put(this.getVarName(classTypeVarBase, counter),
            new ArrayList<String>(Arrays.asList(classType)));
