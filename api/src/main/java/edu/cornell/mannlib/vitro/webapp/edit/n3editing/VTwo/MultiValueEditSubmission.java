...
public class MultiValueEditSubmission {
    ...
    public MultiValueEditSubmission(VitroRequest vreq, EditConfigurationVTwo editConfig) {
        ...
        for (String var : editConfig.getLiteralsOnForm()) {
            ...
            if (field == null) {
                ...
                log.debug(
                    "skipping field with edit element, it should not be in literals on form list");
            } else {
                String[] valuesArray = queryParameters.get(var);
    ...
    public Literal createLiteral_ORIG(String value, String datatypeUri, String lang) {
        if (datatypeUri != null) {
            if ("http://www.w3.org/2001/XMLSchema:anyURI".equals(datatypeUri)) {
                try {
                    return literalCreationModel
                        .createTypedLiteral(URLEncoder.encode(value, "UTF8"), datatypeUri);
    ...
    public Literal getTime(Map<String, String[]> queryParameters, String fieldName) {
        ...
        String hourStr =
            (hourInt < 10) ? "0" + Integer.toString(hourInt) : Integer.toString(hourInt);
        String minuteStr =
            (minuteInt < 10) ? "0" + Integer.toString(minuteInt) : Integer.toString(minuteInt);
        String secondStr = "00";
    ...
    public void addLiteralToForm(EditConfigurationVTwo editConfig, FieldVTwo field, String var,
                                 String[] valuesArray) {
                    ...
                    try {
                        if (_vreq != null) {
                            ...
                            if (getLabelLanguage && isLangSetInFirstLiteral(editConfig)) {
                                rangeLang =
                                    editConfig.getLiteralsInScope().get(LABEL).get(0).getLanguage();
