...
public class Csv2Rdf {
    ...
    public Csv2Rdf(char separatorChar, char quoteChar, String namespace, String tboxNamespace,
                   String typeName) {
    ...
    public Model[] convertToRdf(InputStream fis, WebappDaoFactory wadf, Model destination)
        throws IOException {
        ...
        for (CSVRecord cRecord : cReader) {
            if (dpArray == null) {
                ...
                for (int i = 0; i < dpArray.length; i++) {
                    dpArray[i] = tboxOntModel.createDatatypeProperty(
                        tboxNamespace + propertyNameBase + cRecord.get(i).replaceAll("\\W", ""));
                }
            } else {
                Individual ind = null;
                ...
                for (int col = 0; col < cRecord.size() && col < dpArray.length; col++) {
                    ...
                    if (value.length() > 0) {
                        ind.addProperty(dpArray[col],
                            value); // no longer using: , XSDDatatype.XSDstring);
        ...
        public String getNextURI() {
            ...
            if (namespace != null && !namespace.isEmpty()) {
                while (!uriIsGood && attempts < 30) {
                    uri = namespace + individualNameBase + random
                        .nextInt(Math.min(Integer.MAX_VALUE, (int) Math.pow(2, attempts + 13)));
