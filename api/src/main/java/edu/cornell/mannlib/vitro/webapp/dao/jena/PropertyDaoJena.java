...
public class PropertyDaoJena extends JenaBaseDao implements PropertyDao {
    ...
    static {
        ...
        for (String key : NAMESPACES.keySet()) {
            prefixes.append("PREFIX ").append(key).append(": <").append(NAMESPACES.get(key))
                .append(">\n");
        }
    ...
    public List<String> getEquivalentPropertyURIs(String propertyURI) {
        ...
        getOntModel().enterCriticalSection(Lock.READ);
        try {
            StmtIterator eqStmtIt = getOntModel()
                .listStatements(getOntModel().getResource(propertyURI), OWL.equivalentProperty,
                    (RDFNode) null);
    ...
    public void removeEquivalentProperty(String propertyURI, String equivalentPropertyURI) {
        if (propertyURI == null || equivalentPropertyURI == null) {
            throw new RuntimeException(
                "cannot remove equivalence axiom about anonymous properties");
        }
        OntModel ontModel = getOntModel();
        ontModel.enterCriticalSection(Lock.WRITE);
        try {
    ...
    protected void removeABoxStatementsWithPredicate(String predicateURI) {
        ...
        try {
            aboxModel.getBaseModel()
                .notifyEvent(new EditEvent(getWebappDaoFactory().getUserURI(), true));
            aboxModel
                .removeAll((Resource) null, aboxModel.getProperty(predicateURI), (RDFNode) null);
        } finally {
            aboxModel.getBaseModel()
    ...
    public List<VClass> getClassesWithRestrictionOnProperty(String propertyURI) {
        ...
        try {
            ...
            if (targetProp != null) {
                ...
                StmtIterator stmtIter =
                    ontModel.listStatements((Resource) null, OWL.onProperty, targetProp);
                ...
                while (stmtIter.hasNext()) {
                    ...
                    if (statement.getSubject().canAs(OntClass.class)) {
                        classURISet.addAll(
                            getRestrictedClasses(statement.getSubject().as(OntClass.class)));
                    } else {
                        log.warn(
        ...
        while (iter.hasNext()) {
            ...
            if (vc != null) {
                ...
                log.error(
                    "getClassesWithRestrictionOnProperty: Error: no VClass found for URI: " + curi);
            }
        }
                    ...
                    for (Restriction rest : relatedRestrictions) {
                        ...
                        if (onProperty != null) {
                            ...
                            if (rest.isAllValuesFromRestriction()) {
                                ...
                                updatePropertyRangeMap(applicableProperties, onProperty.getURI(),
                                    ranges, true);
                            } else if (rest.isSomeValuesFromRestriction()) {
                                ranges[1] =
                                    (rest.asSomeValuesFromRestriction()).getSomeValuesFrom();
                                updatePropertyRangeMap(applicableProperties, onProperty.getURI(),
                                    ranges, false);
                            }
                        }
...
//            pi.setRangeClassURI(OWL.Thing.getURI()); // TODO see above
    ...
    private List<ObjectProperty> getAdditionalFauxSubpropertiesForVClasses(
        ...
        StringBuilder propQuery =
            new StringBuilder("PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> \n" +
        ...
        for (PropertyInstance propInst : propInsts) {
            propQuery.append("    FILTER (?property != <").append(propInst.getPropertyURI())
                .append("> ) \n");
        }
    ...
    private List<FullPropertyKey> getAdditionalFauxSubpropertyKeysForPropertyURI(
        String propertyURI) {
