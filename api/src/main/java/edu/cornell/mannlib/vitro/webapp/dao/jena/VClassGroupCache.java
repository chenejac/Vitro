...
public class VClassGroupCache implements SearchIndexer.Listener {
    ...
    public static VClassGroupsForRequest getVClassGroups(HttpServletRequest req) {
        return new VClassGroupsForRequest(req,
            getVClassGroupCache(req.getSession().getServletContext()));
    }

    /**
     * Method that rebuilds the cache. This will use a WebappDaoFactory and a SearchEngine.
     */
    protected static void rebuildCacheUsingSearch(VClassGroupCache cache)
        ...
        VClassGroupDao vcgDao =
            new WebappDaoFactoryFiltering(wdFactory, vFilters).getVClassGroupDao();
    ...
    protected synchronized void setCache(List<VClassGroup> newGroups,
        ...
        protected void checkAndDoUpdate(Statement stmt) {
            ...
            if (RDF.type.equals(stmt.getPredicate())) {
                ...
                OntModel jenaOntModel =
                    ModelAccess.on(context).getOntModelSelector().getTBoxModel();
