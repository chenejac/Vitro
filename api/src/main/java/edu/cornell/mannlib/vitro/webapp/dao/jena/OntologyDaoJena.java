...
public class OntologyDaoJena extends JenaBaseDao implements OntologyDao {
        ...
        try {
            ...
            try {
                while (ontIt.hasNext()) {
                    ...
                    if (!(NONUSER_NAMESPACES.contains(ontRes.getURI()) ||
                        NONUSER_NAMESPACES.contains(ontRes.getURI() + "#"))) {
    ...
    public Ontology getOntologyByURI(String ontologyURI) {
        ...
        if (o == null) {
            try {
                o = ontologyFromOntologyResource(
                    getOntology(adjustOntologyURI(ontologyURI), getOntModel()));
            } catch (Exception e) {
            }
        }
        return o;
        ...
        try {
            ...
            try {
                ...
                if (ontology.getPrefix() != null && ontology.getPrefix().length() > 0) {
                    addPropertyStringValue(o, ONTOLOGY_PREFIX_ANNOT, ontology.getPrefix(),
                        ontModel);
                }
                return o.getURI();
            } finally {
    ...
    public void updateOntology(Ontology ontology, OntModel ontModel) {
        ...
        if (ontology != null && ontology.getURI() != null && ontology.getURI().length() > 0) {
            try {
                org.apache.jena.ontology.Ontology o = getOntology(ontology.getURI(), ontModel);
                if (o == null) {
                    log.error("OntologyDaoJena.updateOntology() could not find ontology " +
                        ontology.getURI() + " in Jena model");
                } else {
    ...
    private Ontology ontologyFromOntologyResource(OntResource ontRes) {
        ...
        if (!(
            ontology.getURI().substring(ontology.getURI().length() - 1, ontology.getURI().length())
                .equals("#") ||
                ontology.getURI()
                    .substring(ontology.getURI().length() - 1, ontology.getURI().length())
                    .equals("/")
        )) {
