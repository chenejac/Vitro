    ...
    public String getUnusedNewURI(String prefixURI) throws InsertException {
        ...
        while (!goodNewURI && attempts < MAX_ATTEMPTS) {
            ...
            if (attempts > 2 && prefixURI != null && !prefixURI.isEmpty()) {
                log.debug("Attempts: " + attempts + " and prefix not null and prefix not empty " +
                    prefixURI);
            ...
            log.debug("Created new uri " + newURI + " and does madeURIs contain it?" +
                madeURIs.contains(newURI));
            if (newURI != null && !newURI.isEmpty() && !madeURIs.contains(newURI)) {
                log.debug(
                    "new URI is not null and new URI is empty and madeURIs does not containt new URI");
                goodNewURI = true;
                madeURIs.add(newURI);
            }
