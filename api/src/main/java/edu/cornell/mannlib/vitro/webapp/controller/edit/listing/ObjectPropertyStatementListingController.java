...
@WebServlet(name = "ObjectPropertyStatementListingController", urlPatterns = {
...
public class ObjectPropertyStatementListingController extends
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (displayParam != null && displayParam.equalsIgnoreCase("true")) {
            showVClasses =
                true;  // this will trigger a limitation to asserted vclasses, since we can't easily display all vclasses for an individual
        }
        ...
        if (assertedStatementsOnly) { // get only asserted, not inferred, object property statements
            opsDao =
                vrequest.getUnfilteredAssertionsWebappDaoFactory().getObjectPropertyStatementDao();
        } else {
        ...
        for (ObjectPropertyStatement objectPropertyStatement : opsDao
            .getObjectPropertyStatements(op, startAt, endAt)) {
            count++;
            ...
            if (showVClasses) {
                try {
                    results
                        .add(ListingControllerWebUtils.formatVClassLinks(subj.getVClasses(true)));
                } catch (Exception e) {
                    results.add("?");
                }
            }
        ...
        ButtonForm newPropButton =
            new ButtonForm(Controllers.RETRY_URL, "buttonForm", "Add new object property",
                newPropParams);
        buttons.add(newPropButton);
        HashMap<String, String> rootPropParams = new HashMap<String, String>();
        rootPropParams.put("iffRoot", "true");
        ButtonForm rootPropButton =
            new ButtonForm("showObjectPropertyHierarchy", "buttonForm", "root properties",
                rootPropParams);
