...
import edu.cornell.mannlib.vitro.webapp.dynapi.io.converters.IOMessageConverter;
...
public class SPARQLQuery extends Operation {
    ...
    public OperationResult run(OperationData inputOutput) {
        ...
        for (String paramName : requiredParams.getNames()) {
            pss.setLiteral(paramName, inputOutput.getData(paramName).toString(), requiredParams.get(paramName).getRDFDataType());
        }
                ...
                while (results.hasNext()) {
                    ...
                    for (String var : vars) {
                        if (solution.contains(var)) {
                            ...
                            Parameter parameter = providedParams.get(fieldName);
                            if (parameter != null)
                                inputOutput.add(fieldName, IOMessageConverterUtils.getPrimitiveDataFromString(solution.get(var).toString(), parameter.getType()));
                        }
                    }
                    j++;
                }
