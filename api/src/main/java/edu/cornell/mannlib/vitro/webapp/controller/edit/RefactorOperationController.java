...
public class RefactorOperationController extends BaseEditController {
    ...
    private String doFixDataTypes(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            while (dataProperties.hasNext()) // Iterate through all datatype properties
            {
                ...
                while (n.hasNext()) // Iterate through all objects of all datatype properties
                {
                    ...
                    if (node.isLiteral()) {
                        if (range !=
                            null) // If a literal has a predicate with a defined range, check and fix the literal's datatype
                        {
                            Literal l = (Literal) node;
                            StmtIterator usingPandL = ontModel.listStatements(null, p, l);
                            int size = 0;
                            results.add(
                                "Statements using property " + p.getLocalName() + " and literal " +
                                    l.getLexicalForm() + ":");
                            while (usingPandL.hasNext()) {
                                Statement st = usingPandL.nextStatement();
                                results.add("    " + st.getSubject().getLocalName() + " " +
                                    p.getLocalName() + " " + l.getLexicalForm());
                                size++;
                            }
                            usingPandL.close();
                            ...
                            if (valid) {
                                ...
                                results.add(p.getLocalName() + " has object " + l.getLexicalForm() +
                                    " of type " + l.getDatatypeURI() + " which is inconsistent");
                                String typeName = "";
                                if (range.getURI().contains(XSDDatatype.XSD)) {
                                    typeName =
                                        range.getURI().substring(XSDDatatype.XSD.length() + 1);
                                } else {
                                ...
                                try {
                                    ...
                                    results.add("ERROR: Can't create XSDDatatype for literal " +
                                        l.getLexicalForm());
                                }
                                ...
                                while (badStatements.hasNext()) {
                                    ...
                                    Statement goodState = ontModel
                                        .createStatement(badState.getSubject(), p, newLiteral);
                                    queue.add(goodState);
                            ...
                            if (valid) {
                                results.add(
                                    "Literal " + l.getLexicalForm() +
                                        " is in the range of property " +
                                        p.getLocalName());
                            }
                            results.add("--------------");
                        }
            ...
            results.add("Of the statements that contain datatype properties with defined ranges, " +
                consistent + " are consistent and " + fixed + " are inconsistent.");
        ...
        try {
            JSPPageHandler
                .renderBasicPage(request, response, Controllers.CHECK_DATATYPE_PROPERTIES);
        } catch (Exception e) {
    ...
    private String doRenameResource(VitroRequest request, HttpServletResponse response,
        ...
        String queryStr =
            "SELECT distinct ?graph WHERE {{ GRAPH ?graph { ?subj <" + oldURIStr + "> ?obj }} ";
        ...
        if ((refererStr = epo.getReferer()) != null) {
            ...
            String[] controllers =
                {"entityEdit", "propertyEdit", "datapropEdit", "ontologyEdit", "vclassEdit"};
    ...
    private void renameResourceInModel(Model model, String userURI, String oldURIStr,
                                       String newURIStr, boolean doNotify) {
        ...
        try {
            Property prop =
                model.getProperty(oldURIStr); // this will create a resource if there isn't
    ...
    private void doMovePropertyStatements(VitroRequest request, HttpServletResponse response,
                                          EditProcessObject epo) {
        String userURI = LoginStatusBean.getBean(request).getUserURI();
        ...
        try {
            Resource subjClass = (subjectClassURIStr.equals("") ? null :
                ResourceFactory.createResource(subjectClassURIStr));
            Property prop = ResourceFactory.createProperty(oldURIStr);
            Property newProp =
                (newURIStr.equals("")) ? null : ResourceFactory.createProperty(newURIStr);
            ...
            try {
                for (StmtIterator stmtIt = closeIt; stmtIt.hasNext(); ) {
                    ...
                    if (moveIt) {
                        ...
                        if (propInv != null) {
                            tempRetractModel
                                .add((Resource) stmt.getObject(), propInv, stmt.getSubject());
                        }
                        if (newProp != null) {
                            tempAddModel.add(stmt.getSubject(), newProp, stmt.getObject());
                            if (newPropInv != null) {
                                tempAddModel.add((Resource) stmt.getObject(), newPropInv,
                                    stmt.getSubject());
                            }
                        }
                    }
    ...
    private void doMoveInstances(VitroRequest request, HttpServletResponse response,
                                 EditProcessObject epo) {
        String userURI = LoginStatusBean.getBean(request).getUserURI();
        ...
        try {
            ...
            Resource newClassRes =
                (newClassURIStr.equals("")) ? null : ontModel.getResource(newClassURIStr);
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse response) {
        ...
        if (epo == null) {
            ...
            modeStr = (String) epo.getAttribute("modeStr");
        }
    ...
    private void handleConsistencyCheckRequest(HttpServletRequest req,
                                               HttpServletResponse response) {
        String modeStr = req.getParameter("modeStr");
        if (modeStr != null) {
            if (modeStr.equals("fixDataTypes")) {
                doFixDataTypes(req, response);
            }
        }
    }
