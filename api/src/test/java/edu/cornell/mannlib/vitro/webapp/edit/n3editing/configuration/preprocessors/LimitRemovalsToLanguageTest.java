...
public class LimitRemovalsToLanguageTest extends AbstractTestClass {
    ...
    public void testPreprocess() {
        ...
        Assert.assertTrue(retractions
            .contains(res, RDFS.label, ResourceFactory.createLangLiteral("en-US1", "en-US")));
        Assert.assertEquals(additions.size(), 1);
        Assert.assertTrue(additions
            .contains(res, RDFS.label, ResourceFactory.createLangLiteral("en-US2", "en-US")));
        additions.removeAll();
        ...
        Assert.assertTrue(retractions
            .contains(res, RDFS.label, ResourceFactory.createLangLiteral("en-US1", "en-US")));
        Assert.assertTrue(
            retractions.contains(res, RDFS.label, ResourceFactory.createLangLiteral("es1", "es")));
        ...
        Assert.assertTrue(additions
            .contains(res, RDFS.label, ResourceFactory.createLangLiteral("en-US2", "en-US")));
        Assert.assertTrue(
            additions.contains(res, RDFS.label, ResourceFactory.createLangLiteral("es2", "es")));
        additions.removeAll();
        retractions.removeAll();
    }
