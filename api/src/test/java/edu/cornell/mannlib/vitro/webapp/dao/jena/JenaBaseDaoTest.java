...
public class JenaBaseDaoTest extends AbstractTestClass {
    ...
    String nosePropIsDependentRel =
        "<" + VitroVocabulary.PROPERTY_STUBOBJECTPROPERTYANNOT +
    ...
    public void smartRemoveTestForIndivdiualDelete() {
        ...
        Model readInModel =
            (ModelFactory.createDefaultModel()).read(new StringReader(n3), "", "N3");
        ...
        try {
            ObjectProperty prop1 =
                wdfj.getObjectPropertyDao().getObjectPropertyByURI("http://example.com/prop1");
            ...
            Model expectedModel =
                (ModelFactory.createOntologyModel()).read(new StringReader(expected), "", "N3");
...
//	}
    ...
    public void testABoxAssertionsRemovedWhenPropertyRemoved() throws InsertException {
        ...
        preWadf.getObjectPropertyStatementDao()
            .insertNewObjectPropertyStatement(objPropStmtNotForDeletion);
        ...
        preWadf.getObjectPropertyStatementDao()
            .insertNewObjectPropertyStatement(objPropStmtForDeletion);
        ...
        preWadf.getDataPropertyStatementDao()
            .insertNewDataPropertyStatement(dataPropStmtForDeletion);
    ...
    public void testUpdateRDFSLabel() {
        ...
        Assert.assertTrue(m.contains(ontRes, RDFS.label,
            ResourceFactory.createLangLiteral("engLabel2", "en-US")));
        Assert.assertTrue(
            m.contains(ontRes, RDFS.label, ResourceFactory.createLangLiteral("esLabel1", "es-ES")));
        m.removeAll();
        ...
        Assert.assertTrue(m.contains(ontRes, RDFS.label,
            ResourceFactory.createLangLiteral("engLabel1", "en-US")));
        Assert.assertTrue(
            m.contains(ontRes, RDFS.label, ResourceFactory.createLangLiteral("esLabel1", "es-ES")));
